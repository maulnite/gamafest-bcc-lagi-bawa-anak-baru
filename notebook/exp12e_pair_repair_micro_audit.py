# %% [markdown]
# # 00. EXP12E — Pair-Repair Policy Search, Native Pair Consensus, Micro Calibration, dan Local Audit
#
# Penjelasan bagian:
# Script ini melanjutkan EXP12D/EXP22 dengan fokus pada pair-repair policy search, native pair consensus, micro calibration kecil, dan optional local GT audit di cell terakhir. Modeling tetap mengadaptasi EXP22/EXP12D, tetapi output path dan guardrail mengikuti struktur project user.
#
...
EXP12D_VARIANT = os.environ.get("EXP12D_CORE_VARIANT", os.environ.get("EXP12D_VARIANT", "d1_exp15_first"))
EXP12E_VARIANT = os.environ.get("EXP12E_VARIANT", "e1_pair_repair_policy_search")
...
ABLATION_ROOT_DIR = OUTPUT_ROOT / "exp12e_pair_repair_micro_audit"
OUT_DIR = ABLATION_ROOT_DIR / EXP12E_VARIANT
...
print(f"[INFO] EXP12E_VARIANT = {EXP12E_VARIANT}", flush=True)
...
FAST_MODE = os.environ.get(
    "EXP12E_FAST_MODE",
    os.environ.get("EXP12D_FAST_MODE", os.environ.get("EXP12C_FAST_MODE", "0"))
...
# EXP12D core best_safe must be pair-consistent before strict save.
# Raw EXP15/D1 can contain mirrored-row mismatches, so repair with the same
# mean-round policy used by the EXP12D best_safe anchor before validation.
best_source_raw = candidate_sources[selected_key]

pair_ok_before, bad_before = _pair_consistency_exp12d(
    align_to_sample_exp12d(best_source_raw),
    test_raw,
)

if not pair_ok_before:
    log_info(f"Repairing EXP12D best_safe pair consistency: bad_pairs_before={bad_before}")

    sub_tmp = align_to_sample_exp12d(best_source_raw)
    row_info = test_raw[[ID_COL, MATCH_COL]].copy()
    row_info["_id_key"] = row_info[ID_COL].astype(str)

    sub_tmp["_id_key"] = sub_tmp[ID_COL].astype(str)
    merged_tmp = sub_tmp.merge(
        row_info[["_id_key", MATCH_COL]],
        on="_id_key",
        how="left",
        validate="one_to_one",
    )
    ...
    for match_id, g in merged_tmp.groupby(MATCH_COL, sort=False):
        ...
        if len(g) != 2:
            for _, r in g.iterrows():
                repaired_rows.append({
                    ID_COL: r[ID_COL],
                    "team_goals": int(r["team_goals"]),
                    "opp_goals": int(r["opp_goals"]),
                })
            continue
        ...
        a_goals = int(round((int(r0["team_goals"]) + int(r1["opp_goals"])) / 2.0))
        b_goals = int(round((int(r0["opp_goals"]) + int(r1["team_goals"])) / 2.0))

        a_goals = max(0, a_goals)
        b_goals = max(0, b_goals)
    ...
    best_source_final = pd.DataFrame(repaired_rows)
    best_source_final = align_to_sample_exp12d(best_source_final)

else:
    best_source_final = best_source_raw

pair_ok_after, bad_after = _pair_consistency_exp12d(
    align_to_sample_exp12d(best_source_final),
    test_raw,
)
...
pd.DataFrame([{
    ...
    "bad_pairs_before": int(bad_before),
    "bad_pairs_after": int(bad_after),
    "pair_repair_applied": bool(not pair_ok_before),
}]).to_csv(SUM_DIR / "pair_repair_report_best_safe.csv", index=False)
log_saved(SUM_DIR / "pair_repair_report_best_safe.csv")
...
print("[EXP12D CORE SUMMARY - before EXP12E policy search]", flush=True)
...
# %% [markdown]
# # 18. EXP12E — Pair-Repair Policy Search
#
# Penjelasan bagian:
# Bagian ini mengambil raw EXP15/EXP12D candidate, lalu mencoba beberapa policy pair repair. Fokusnya bukan training ulang model besar, melainkan mencari cara paling aman untuk mengubah dua row mirror menjadi satu prediksi match-level yang konsisten.
#
# Output yang perlu dilihat:
# Lihat `pair_repair_policy_grid.csv`, `pair_repair_transition_analysis.csv`, dan submission `submission_exp12e_e1_*.csv`. Candidate aman harus `pair_consistency=True`, `bad_pairs_after=0`, changed rate wajar, dan mean predicted total tidak berubah liar.

# %%
# ============================================================
# 18. EXP12E pair-repair policy search and micro candidates
# ============================================================

log_section("EXP12E pair-repair policy search")

# EXP12E uses the EXP22/EXP12D core objects created above, but saves with EXP12E names.
EXP12E_OUT_DIR = OUT_DIR
EXP12E_SUM_DIR = SUM_DIR
EXP12E_PRED_DIR = PRED_DIR
EXP12E_SUB_DIR = SUB_DIR
EXP12E_FIG_DIR = FIG_DIR

for _d in [EXP12E_SUM_DIR, EXP12E_PRED_DIR, EXP12E_SUB_DIR, EXP12E_FIG_DIR]:
    _d.mkdir(parents=True, exist_ok=True)


def validate_and_save_submission_exp12e(sub_df: pd.DataFrame, label: str, strict: bool = True):
    """Validate and save EXP12E submission with EXP12E naming."""
    aligned = align_to_sample_exp12d(sub_df)
    pair_pass, n_bad_pairs = _pair_consistency_exp12d(aligned, test_raw)
    checks = [
        {"variant": label, "check": "shape_matches_sample", "passed": tuple(aligned.shape) == (len(sample_submission), 3), "detail": str(tuple(aligned.shape))},
    ...
    check_df = pd.DataFrame(checks)
    save_df = aligned.rename(columns={ID_COL: sample_id_col})[[sample_id_col, "team_goals", "opp_goals"]].copy()
    path = EXP12E_SUB_DIR / f"submission_exp12e_{label}.csv"
    save_df.to_csv(path, index=False)
    log_saved(path)
    if strict and not bool(check_df["passed"].all()):
        display(check_df)
        raise RuntimeError(f"EXP12E submission validation failed for {label}")
    return aligned, path, check_df


def _orientation_rows_for_match(g: pd.DataFrame):
    """Return row0 and row1 from a two-row mirrored match group."""
    g = g.sort_values(ID_COL).reset_index(drop=True)
    if len(g) != 2:
        return None, None
...
def pair_repair_submission(sub_df: pd.DataFrame, policy: str = "repair_mean_round") -> pd.DataFrame:
    """Build pair-consistent submission from row-level possibly-inconsistent predictions.

    Canonical side A follows first row in each match group. Side B is the mirrored second row.
    """
    sub = align_to_sample_exp12d(sub_df)
    row_info = test_raw[[ID_COL, MATCH_COL, "gender", "tournament"]].copy()
    row_info["_id_key"] = row_info[ID_COL].astype(str)
    sub["_id_key"] = sub[ID_COL].astype(str)
    merged = sub.merge(row_info[["_id_key", MATCH_COL, "gender", "tournament"]], on="_id_key", how="left", validate="one_to_one")
    ...
    records = []
    for match_id, g in merged.groupby(MATCH_COL, sort=False):
        r0, r1 = _orientation_rows_for_match(g)
        if r0 is None:
            continue

        a0 = int(r0["team_goals"])
        b0 = int(r0["opp_goals"])
        # mirrored view from row 1: A goals are row1 opp_goals, B goals are row1 team_goals
        a1 = int(r1["opp_goals"])
        b1 = int(r1["team_goals"])
        ...
        if policy == "repair_trust_row_a":
            a, b = orient0
        elif policy == "repair_trust_row_b":
            a, b = orient1
        elif policy == "repair_mean_floor":
            a, b = int(math.floor((a0 + a1) / 2)), int(math.floor((b0 + b1) / 2))
        elif policy == "repair_mean_ceil":
            a, b = int(math.ceil((a0 + a1) / 2)), int(math.ceil((b0 + b1) / 2))
        elif policy == "repair_lower_total":
            a, b = orient0 if sum(orient0) <= sum(orient1) else orient1
        elif policy == "repair_higher_total":
            a, b = orient0 if sum(orient0) >= sum(orient1) else orient1
        elif policy == "repair_preserve_gd_lower_total":
            gd0, gd1 = a0 - b0, a1 - b1
            if np.sign(gd0) == np.sign(gd1):
                a, b = orient0 if sum(orient0) <= sum(orient1) else orient1
            else:
                a, b = int(round((a0 + a1) / 2)), int(round((b0 + b1) / 2))
        elif policy == "repair_min_total_same_outcome":
            if np.sign(a0 - b0) == np.sign(a1 - b1):
                a, b = orient0 if sum(orient0) <= sum(orient1) else orient1
            else:
                a, b = int(round((a0 + a1) / 2)), int(round((b0 + b1) / 2))
        elif policy == "repair_max_total_same_outcome":
            if np.sign(a0 - b0) == np.sign(a1 - b1):
                a, b = orient0 if sum(orient0) >= sum(orient1) else orient1
            else:
                a, b = int(round((a0 + a1) / 2)), int(round((b0 + b1) / 2))
        else:  # repair_mean_round
            a, b = int(round((a0 + a1) / 2)), int(round((b0 + b1) / 2))
        ...
        records.append({
            ...
            "gender": r0.get("gender", np.nan),
            "tournament": r0.get("tournament", np.nan),
            "pred_team_a_goals": max(0, int(a)),
            "pred_team_b_goals": max(0, int(b)),
        })
    match_df = pd.DataFrame(records)
    return match_df_to_submission_exp12d(match_df)


def pair_repair_report(before_sub: pd.DataFrame, after_sub: pd.DataFrame, label: str) -> pd.DataFrame:
    before_pass, before_bad = _pair_consistency_exp12d(before_sub, test_raw)
    after_pass, after_bad = _pair_consistency_exp12d(after_sub, test_raw)
    ch = changed_analysis(before_sub, after_sub, label)
    rec = ch.iloc[0].to_dict()
    rec.update({
        "variant": label,
        "bad_pairs_before": int(before_bad),
        "bad_pairs_after": int(after_bad),
        "pair_pass_before": bool(before_pass),
        "pair_pass_after": bool(after_pass),
    })
    return pd.DataFrame([rec])

# Raw EXP15 remains the primary inconsistent source for repair-policy search.
raw_exp15_sub = candidate_sources.get("d1_exp15_first", exp15_base_sub)
raw_exp17_sub = candidate_sources.get("d2_exp17_original", raw_exp15_sub)

# E0: EXP12D best_safe reproduction anchor.
# EXP12D best_safe sebelumnya berasal dari raw EXP15 yang di-repair pair-consistency
# dengan mean-round. Jangan ambil d0_final_candidate karena itu bisa mengarah ke
# EXP17/final candidate yang berbeda.
e0_reproduction = pair_repair_submission(raw_exp15_sub, "repair_mean_round")
e0_pair_pass, e0_bad = _pair_consistency_exp12d(e0_reproduction, test_raw)
assert e0_pair_pass, f"E0 reproduction is not pair-consistent, bad_pairs={e0_bad}"

e0_note = "forced_repair_mean_round_from_raw_exp15_to_reproduce_EXP12D_best_safe"
log_info(f"E0 reproduction source: {e0_note}")

EXP12E_CANDIDATES = {"e0_exp12d_reproduction": e0_reproduction}

PAIR_REPAIR_POLICIES = [
    "repair_mean_round",
    "repair_mean_floor",
    "repair_mean_ceil",
    "repair_lower_total",
...
pair_repair_rows = []
transition_frames = []
for pol in PAIR_REPAIR_POLICIES:
    cand = pair_repair_submission(raw_exp15_sub, pol)
    key = f"e1_{pol}"
    EXP12E_CANDIDATES[key] = cand
    report = pair_repair_report(raw_exp15_sub, cand, key)
    metrics = gt_free_metrics(cand, key, e0_reproduction).iloc[0].to_dict()
    pair_repair_rows.append({**report.iloc[0].to_dict(), **{f"metric_{k}": v for k, v in metrics.items() if k != "variant"}})
    transition_frames.append(changed_analysis(e0_reproduction, cand, key))

pair_repair_policy_grid = pd.DataFrame(pair_repair_rows)
pair_repair_policy_grid.to_csv(EXP12E_SUM_DIR / "pair_repair_policy_grid.csv", index=False)
log_saved(EXP12E_SUM_DIR / "pair_repair_policy_grid.csv")
display(pair_repair_policy_grid)

pair_repair_transition_analysis = pd.concat(transition_frames, ignore_index=True)
pair_repair_transition_analysis.to_csv(EXP12E_SUM_DIR / "pair_repair_transition_analysis.csv", index=False)
log_saved(EXP12E_SUM_DIR / "pair_repair_transition_analysis.csv")
display(pair_repair_transition_analysis)

# Save E0 and selected E1 artifacts now.
e0_aligned, e0_path, e0_check = validate_and_save_submission_exp12e(e0_reproduction, "e0_exp12d_reproduction", strict=True)

for pol in ["repair_mean_round", "repair_lower_total", "repair_preserve_gd_lower_total", "repair_min_total_same_outcome", "repair_trust_row_a", "repair_trust_row_b"]:
    key = f"e1_{pol}"
    validate_and_save_submission_exp12e(EXP12E_CANDIDATES[key], key, strict=False)

# %% [markdown]
# # 19. EXP12E — Native Pair Consensus
#
# Penjelasan bagian:
# Bagian ini membuat consensus langsung di match-level. Secara konsep ini mirip native pair decode: dua orientasi mirror digabung, lalu dipilih satu scoreline per match sebelum dikembalikan ke submission row-level.
#
# Output yang perlu dilihat:
# Lihat `native_pair_consensus_grid.csv` dan submission `e2_*`. Semua output harus pair-consistent tanpa repair tambahan. Jika native consensus mengalahkan repair policy pada audit lokal, arah eksperimen berikutnya bisa dibuat fully match-level.

# %%
log_section("EXP12E native pair consensus")

NATIVE_CONSENSUS_POLICIES = {
    "consensus_avg_pmf": "repair_mean_round",
    "consensus_raw_row_a": "repair_trust_row_a",
    "consensus_raw_row_b": "repair_trust_row_b",
    "consensus_lower_total": "repair_lower_total",
    "consensus_outcome_agree_only": "repair_min_total_same_outcome",
}

native_rows = []
for native_name, repair_policy in NATIVE_CONSENSUS_POLICIES.items():
    cand = pair_repair_submission(raw_exp15_sub, repair_policy)
    key = f"e2_{native_name}"
    EXP12E_CANDIDATES[key] = cand
    rep = pair_repair_report(raw_exp15_sub, cand, key).iloc[0].to_dict()
    met = gt_free_metrics(cand, key, e0_reproduction).iloc[0].to_dict()
    native_rows.append({**rep, **{f"metric_{k}": v for k, v in met.items() if k != "variant"}})
    validate_and_save_submission_exp12e(cand, key, strict=False)

native_pair_consensus_grid = pd.DataFrame(native_rows)
native_pair_consensus_grid.to_csv(EXP12E_SUM_DIR / "native_pair_consensus_grid.csv", index=False)
log_saved(EXP12E_SUM_DIR / "native_pair_consensus_grid.csv")
display(native_pair_consensus_grid)

# %% [markdown]
# # 20. EXP12E — Micro Bias Alignment
#
# Penjelasan bagian:
# Bagian ini mencoba bias alignment kecil dengan changed cap 2%, 5%, 8%, dan 10%. Berbeda dari calibration agresif lama, perubahan hanya diterapkan pada subset low-risk dan tetap dalam representasi match-level.
#
# Output yang perlu dilihat:
# Lihat `micro_bias_grid.csv` dan `scoreline_transition_e3.csv`. Candidate aman harus changed rate sesuai cap, pair consistency true, dan mean predicted total tidak bergeser ekstrem.

# %%
log_section("EXP12E micro bias alignment")

def _apply_match_transition_limited(base_sub: pd.DataFrame, transition_specs: list[tuple[int, int, int, int]], cap_rate: float, label: str) -> pd.DataFrame:
    match = submission_to_match_df(base_sub, test_raw).copy()
    a = match["pred_team_a_goals"].astype(int).copy()
    b = match["pred_team_b_goals"].astype(int).copy()
    n_cap = max(1, int(math.floor((len(match) * float(cap_rate)))))
    changed = np.zeros(len(match), dtype=bool)

    for old_a, old_b, new_a, new_b in transition_specs:
        if changed.sum() >= n_cap:
            break
        mask = (~changed) & (a.values == old_a) & (b.values == old_b)
        # avoid very high total / extreme candidates for micro policies
        mask = mask & ((a.values + b.values) <= 5)
        idx = np.where(mask)[0]
        if len(idx) == 0:
            continue
        take = idx[: max(0, n_cap - int(changed.sum()))]
        a.iloc[take] = int(new_a)
        b.iloc[take] = int(new_b)
        changed[take] = True

    out = match.copy()
    out["pred_team_a_goals"] = a.clip(lower=0).astype(int)
    out["pred_team_b_goals"] = b.clip(lower=0).astype(int)
    out["micro_policy"] = label
    return match_df_to_submission_exp12d(out)
...
micro_bias_rows = []
micro_bias_transitions = []
for cap in [0.02, 0.05, 0.08, 0.10]:
    label = f"e3_micro_bias_cap{int(cap * 100):02d}"
    cand = _apply_match_transition_limited(e0_reproduction, BIAS_TRANSITIONS, cap, label)
    EXP12E_CANDIDATES[label] = cand
    met = gt_free_metrics(cand, label, e0_reproduction).iloc[0].to_dict()
    ch = changed_analysis(e0_reproduction, cand, label).iloc[0].to_dict()
    micro_bias_rows.append({**met, **{f"changed_{k}": v for k, v in ch.items() if k != "variant"}, "cap_rate": cap})
    micro_bias_transitions.append(changed_analysis(e0_reproduction, cand, label))
    validate_and_save_submission_exp12e(cand, label, strict=False)

micro_bias_grid = pd.DataFrame(micro_bias_rows)
micro_bias_grid.to_csv(EXP12E_SUM_DIR / "micro_bias_grid.csv", index=False)
log_saved(EXP12E_SUM_DIR / "micro_bias_grid.csv")
display(micro_bias_grid)

scoreline_transition_e3 = pd.concat(micro_bias_transitions, ignore_index=True)
scoreline_transition_e3.to_csv(EXP12E_SUM_DIR / "scoreline_transition_e3.csv", index=False)
log_saved(EXP12E_SUM_DIR / "scoreline_transition_e3.csv")
display(scoreline_transition_e3)

# %% [markdown]
# # 21. EXP12E — Micro Exact Booster
#
# Penjelasan bagian:
# Bagian ini mencoba exact booster kecil dengan cap 2% dan 5%. Perubahan dibuat sangat terbatas supaya tidak mengulang eksperimen yang terlalu agresif.
#
# Output yang perlu dilihat:
# Lihat `micro_exact_grid.csv` dan `scoreline_transition_e4.csv`. Candidate aman punya changed rate rendah, mayoritas transisi common scoreline, dan pair consistency true.

# %%
log_section("EXP12E micro exact booster")
...
micro_exact_rows = []
micro_exact_transitions = []
for cap in [0.02, 0.05]:
    label = f"e4_micro_exact_cap{int(cap * 100):02d}"
    cand = _apply_match_transition_limited(e0_reproduction, EXACT_TRANSITIONS, cap, label)
    EXP12E_CANDIDATES[label] = cand
    met = gt_free_metrics(cand, label, e0_reproduction).iloc[0].to_dict()
    ch = changed_analysis(e0_reproduction, cand, label).iloc[0].to_dict()
    micro_exact_rows.append({**met, **{f"changed_{k}": v for k, v in ch.items() if k != "variant"}, "cap_rate": cap})
    micro_exact_transitions.append(changed_analysis(e0_reproduction, cand, label))
    validate_and_save_submission_exp12e(cand, label, strict=False)

micro_exact_grid = pd.DataFrame(micro_exact_rows)
micro_exact_grid.to_csv(EXP12E_SUM_DIR / "micro_exact_grid.csv", index=False)
log_saved(EXP12E_SUM_DIR / "micro_exact_grid.csv")
display(micro_exact_grid)

scoreline_transition_e4 = pd.concat(micro_exact_transitions, ignore_index=True)
scoreline_transition_e4.to_csv(EXP12E_SUM_DIR / "scoreline_transition_e4.csv", index=False)
log_saved(EXP12E_SUM_DIR / "scoreline_transition_e4.csv")
display(scoreline_transition_e4)

# %% [markdown]
# # 22. EXP12E — Micro EXP17 Gate dan Optional EXP12B Hybrid
#
# Penjelasan bagian:
# Bagian ini memakai EXP17 atau EXP12B hanya pada subset agreement yang aman. Jika EXP12B candidate tidak tersedia, E6 akan ditandai skipped agar tidak terlihat seperti improvement palsu.
#
# Output yang perlu dilihat:
# Lihat `micro_exp17_gate_grid.csv` dan `inline_exp12b_hybrid_analysis.csv`. Status E6 harus `active` kalau benar-benar ada sumber EXP12B, atau `skipped` jika tidak tersedia.

# %%
log_section("EXP12E micro EXP17 gate and optional EXP12B hybrid")


def _agreement_gate(base_sub: pd.DataFrame, alt_sub: pd.DataFrame, label: str, cap_rate: float = 0.05, w_only: bool = False) -> pd.DataFrame:
    base_m = submission_to_match_df(base_sub, test_raw).rename(columns={"pred_team_a_goals": "base_a", "pred_team_b_goals": "base_b"})
    alt_m = submission_to_match_df(alt_sub, test_raw).rename(columns={"pred_team_a_goals": "alt_a", "pred_team_b_goals": "alt_b"})
    m = base_m.merge(alt_m[[MATCH_COL, "alt_a", "alt_b"]], on=MATCH_COL, how="left", validate="one_to_one")

    base_out = outcome_array(m["base_a"], m["base_b"])
    alt_out = outcome_array(m["alt_a"], m["alt_b"])
    base_gd = m["base_a"] - m["base_b"]
    alt_gd = m["alt_a"] - m["alt_b"]
    base_total = m["base_a"] + m["base_b"]
    alt_total = m["alt_a"] + m["alt_b"]
    eligible = (base_out == alt_out) & (base_gd == alt_gd) & (np.abs(base_total - alt_total) <= 1)
    if w_only and "gender" in m.columns:
        eligible = eligible & m["gender"].astype(str).str.upper().eq("W")
    eligible = eligible & (base_total <= 5) & (alt_total <= 5)
    changed = eligible & ((m["base_a"] != m["alt_a"]) | (m["base_b"] != m["alt_b"]))
    idx = np.where(changed)[0]
    n_cap = max(1, int(math.floor(len(m) * cap_rate)))
    take = set(idx[:n_cap].tolist())
    out = m.copy()
    use_alt = np.array([i in take for i in range(len(out))])
    out["pred_team_a_goals"] = np.where(use_alt, out["alt_a"], out["base_a"]).astype(int)
    out["pred_team_b_goals"] = np.where(use_alt, out["alt_b"], out["base_b"]).astype(int)
    out["micro_gate_label"] = label
    return match_df_to_submission_exp12d(out[[MATCH_COL, "row_id_a", "row_id_b", "gender", "tournament", "pred_team_a_goals", "pred_team_b_goals"]])

micro_exp17_rows = []
if raw_exp17_sub is not None:
    for cap in [0.02, 0.05]:
        label = f"e5_micro_exp17_gate_cap{int(cap * 100):02d}"
        cand = _agreement_gate(e0_reproduction, raw_exp17_sub, label, cap_rate=cap, w_only=True)
        EXP12E_CANDIDATES[label] = cand
        met = gt_free_metrics(cand, label, e0_reproduction).iloc[0].to_dict()
        ch = changed_analysis(e0_reproduction, cand, label).iloc[0].to_dict()
        micro_exp17_rows.append({**met, **{f"changed_{k}": v for k, v in ch.items() if k != "variant"}, "cap_rate": cap})
        validate_and_save_submission_exp12e(cand, label, strict=False)
else:
    micro_exp17_rows.append({"variant": "e5_micro_exp17_gate", "status": "skipped_no_exp17"})

micro_exp17_gate_grid = pd.DataFrame(micro_exp17_rows)
micro_exp17_gate_grid.to_csv(EXP12E_SUM_DIR / "micro_exp17_gate_grid.csv", index=False)
log_saved(EXP12E_SUM_DIR / "micro_exp17_gate_grid.csv")
display(micro_exp17_gate_grid)

# Optional EXP12B hybrid. This is a comparison artifact, not training signal.
def find_exp12b_candidate_path() -> tuple[Path | None, str]:
    env_path = os.environ.get("EXP12B_CANDIDATE_PATH", "").strip()
    if env_path:
        p = Path(env_path)
        if p.exists():
            return p, "env_EXP12B_CANDIDATE_PATH"
    allow_csv = os.environ.get("EXP12E_ALLOW_EXP12B_CSV_HYBRID", "0").strip() in {"1", "true", "True", "YES", "yes"}
    if not allow_csv:
        return None, "skipped_allow_flag_false"
    roots = [OUTPUT_ROOT / "exp12b_no_pseudo_pair_native_decoder_alignment", PROJECT_ROOT / "outputs" / "exp12b_no_pseudo_pair_native_decoder_alignment"]
    names = ["submission_exp12b_b5_exp15_calibrated.csv", "submission_exp12b_best_safe.csv"]
    for r in roots:
        if not r.exists():
            continue
        for n in names:
            hits = list(r.rglob(n))
            if hits:
                return hits[0], f"auto_found_{n}"
    return None, "not_found"

exp12b_path, exp12b_status = find_exp12b_candidate_path()
if exp12b_path is not None:
    exp12b_candidate = pd.read_csv(exp12b_path)
    e6_candidate = _agreement_gate(e0_reproduction, exp12b_candidate, "e6_exp12b_hybrid_cap05", cap_rate=0.05, w_only=False)
    EXP12E_CANDIDATES["e6_exp12b_hybrid_cap05"] = e6_candidate
    validate_and_save_submission_exp12e(e6_candidate, "e6_exp12b_hybrid_cap05", strict=False)
    e6_analysis = changed_analysis(e0_reproduction, e6_candidate, "e6_exp12b_hybrid_cap05")
    e6_analysis.insert(1, "status", "active")
    e6_analysis.insert(2, "exp12b_source", str(exp12b_path))
else:
    e6_analysis = pd.DataFrame([{"variant": "e6_exp12b_hybrid", "status": "skipped", "exp12b_source": exp12b_status}])

inline_exp12b_hybrid_analysis = e6_analysis
inline_exp12b_hybrid_analysis.to_csv(EXP12E_SUM_DIR / "inline_exp12b_hybrid_analysis.csv", index=False)
log_saved(EXP12E_SUM_DIR / "inline_exp12b_hybrid_analysis.csv")
display(inline_exp12b_hybrid_analysis)

# %% [markdown]
# # 23. EXP12E — Final Selected Safe dan Catalog
#
# Penjelasan bagian:
# Bagian ini menyimpan semua candidate EXP12E, lalu membuat `submission_exp12e_best_safe.csv`. Main pipeline tidak memakai GT untuk selection. Default aman adalah E0 reproduction, sementara semua policy lain disarankan untuk local audit di cell terakhir.
#
# Output yang perlu dilihat:
# Lihat `submission_catalog.csv`, `submission_check.csv`, `variant_metrics.csv`, dan `final_decision.csv`. Semua final candidate harus id-order pass, non-missing, non-negative, dan pair-consistent.

# %%
log_section("EXP12E final selected safe")

# Save all candidates that have not been saved explicitly.
submission_catalog_e_rows = []
submission_check_e_frames = []
for key, sub in EXP12E_CANDIDATES.items():
    # avoid duplicate writes for common already-saved labels; still harmless to overwrite.
    try:
        aligned, path, chk = validate_and_save_submission_exp12e(sub, key, strict=False)
        submission_catalog_e_rows.append({"key": key, "path": str(path), "strict": False})
        submission_check_e_frames.append(chk)
    except Exception as exc:
        submission_catalog_e_rows.append({"key": key, "path": "", "strict": False, "error": repr(exc)})

# GT-free selection: keep E0 as best_safe unless user explicitly selects another generated key.
selected_exp12e_key = os.environ.get("EXP12E_SELECTED_KEY", "e0_exp12d_reproduction")
if selected_exp12e_key not in EXP12E_CANDIDATES:
    log_info(f"EXP12E_SELECTED_KEY={selected_exp12e_key} not available; fallback to e0_exp12d_reproduction")
    selected_exp12e_key = "e0_exp12d_reproduction"

best_e_aligned, best_e_path, best_e_check = validate_and_save_submission_exp12e(
    EXP12E_CANDIDATES[selected_exp12e_key],
    "best_safe",
    strict=True,
)
submission_catalog_e_rows.append({"key": selected_exp12e_key, "path": str(best_e_path), "strict": True, "label": "best_safe"})
submission_check_e_frames.append(best_e_check)

submission_catalog_e = pd.DataFrame(submission_catalog_e_rows).drop_duplicates().reset_index(drop=True)
submission_catalog_e.to_csv(EXP12E_SUM_DIR / "submission_catalog.csv", index=False)
log_saved(EXP12E_SUM_DIR / "submission_catalog.csv")
display(submission_catalog_e)

submission_check_e = pd.concat(submission_check_e_frames, ignore_index=True).drop_duplicates().reset_index(drop=True)
submission_check_e.to_csv(EXP12E_SUM_DIR / "submission_check.csv", index=False)
log_saved(EXP12E_SUM_DIR / "submission_check.csv")
display(submission_check_e)

variant_metric_frames_e = []
scoreline_frames_e = []
changed_frames_e = []
for key, sub in EXP12E_CANDIDATES.items():
    variant_metric_frames_e.append(gt_free_metrics(sub, key, e0_reproduction))
    scoreline_frames_e.append(make_scoreline_distribution(sub, key).head(15))
    if key != "e0_exp12d_reproduction":
        changed_frames_e.append(changed_analysis(e0_reproduction, sub, key))

variant_metrics_e = pd.concat(variant_metric_frames_e, ignore_index=True)
variant_metrics_e.to_csv(EXP12E_SUM_DIR / "variant_metrics.csv", index=False)
log_saved(EXP12E_SUM_DIR / "variant_metrics.csv")
display(variant_metrics_e)

scoreline_distribution_e = pd.concat(scoreline_frames_e, ignore_index=True)
scoreline_distribution_e.to_csv(EXP12E_SUM_DIR / "scoreline_distribution.csv", index=False)
log_saved(EXP12E_SUM_DIR / "scoreline_distribution.csv")
display(scoreline_distribution_e.head(60))

changed_prediction_analysis_e = pd.concat(changed_frames_e, ignore_index=True) if changed_frames_e else pd.DataFrame()
changed_prediction_analysis_e.to_csv(EXP12E_SUM_DIR / "changed_prediction_analysis.csv", index=False)
log_saved(EXP12E_SUM_DIR / "changed_prediction_analysis.csv")
display(changed_prediction_analysis_e)

final_decision_e = {
    "experiment": "EXP12E",
    "selected_by_pipeline": selected_exp12e_key,
    "selected_by_validation": "not_available_gt_free_pipeline",
    "recommended_for_local_audit": "all submission_exp12e_*.csv in this OUT_DIR plus EXP12D/EXP12C/EXP12B candidates",
    "main_submission_path": str(best_e_path),
    "out_dir": str(EXP12E_OUT_DIR),
    "local_audit_cell": "Section 99 reads ground_truth_bersih.csv only after submissions are created.",
    "note": "GT is not used for training/tuning/main pipeline decision. Use final local audit table for manual ranking.",
}
with open(EXP12E_SUM_DIR / "final_decision.json", "w", encoding="utf-8") as f:
    json.dump(final_decision_e, f, indent=2)
final_decision_e_df = pd.DataFrame([{"key": k, "value": str(v)} for k, v in final_decision_e.items()])
final_decision_e_df.to_csv(EXP12E_SUM_DIR / "final_decision.csv", index=False)
log_saved(EXP12E_SUM_DIR / "final_decision.json")
log_saved(EXP12E_SUM_DIR / "final_decision.csv")
display(final_decision_e_df)

print("[EXP12E SUMMARY]", flush=True)
log_info(f"EXP12E_VARIANT: {EXP12E_VARIANT}")
log_info(f"selected_exp12e_key: {selected_exp12e_key}")
log_info(f"best_safe_path: {best_e_path}")
log_info(f"OUT_DIR: {EXP12E_OUT_DIR.resolve()}")
...
#
# Output yang perlu dilihat:
# Lihat ranking AW-MAE, base_mae, exact_rate, outcome_rate, gd_rate, dan bias. Ranking ini membantu membandingkan CSV tanpa download manual. Jangan gunakan cell ini untuk training, feature engineering, pseudo-label, decoder tuning runtime, atau final decision otomatis.

# %%
# ============================================================
# 99. Optional local GT audit. This cell is intentionally last.
# ============================================================

RUN_LOCAL_GT_AUDIT = os.environ.get("EXP12E_RUN_LOCAL_GT_AUDIT", "1").strip() in {"1", "true", "True", "YES", "yes"}
...
if RUN_LOCAL_GT_AUDIT:
    ...
    if GT_PATH is None:
        ...
        gt = pd.read_csv(GT_PATH)
        if "id" in gt.columns and "Id" not in gt.columns:
        ...
        def get_tournament_weight_audit(tournament: str) -> float:
            t = str(tournament).lower().strip()
            if "fifa world cup" in t or t == "world cup":
                return 2.00
            if "afc championship" in t or "afc asian cup" in t or "asian cup" in t:
        ...
        if "test_raw" in globals() and ID_COL in test_raw.columns:
            test_weight_df = test_raw[[ID_COL]].copy().rename(columns={ID_COL: "Id"})
            test_weight_df["Id"] = test_weight_df["Id"].astype(str)
            if "tournament" in test_raw.columns:
                test_weight_df["weight"] = test_raw["tournament"].apply(get_tournament_weight_audit).astype(float)
            else:
                test_weight_df["weight"] = 1.20
        else:
            test_weight_df = gt[["Id"]].copy()
            test_weight_df["weight"] = 1.20

        gt = gt.merge(test_weight_df, on="Id", how="left")
        gt["weight"] = gt["weight"].fillna(1.20).astype(float)

        EXACT_PENALTY_AUDIT = 0.30
        OUTCOME_PENALTY_AUDIT = 0.25
        GD_PENALTY_AUDIT = 0.15
        WRONG_OUTCOME_MULTIPLIER_AUDIT = 1.50
        NONLINEAR_POWER_AUDIT = 1.50

        def awmae_components_audit(y_team, y_opp, p_team, p_opp, weights):
            ...
            raw = base + EXACT_PENALTY_AUDIT * exact_miss.astype(float) + OUTCOME_PENALTY_AUDIT * outcome_miss.astype(float) + GD_PENALTY_AUDIT * gd_miss.astype(float)
            mult = np.where(outcome_miss, WRONG_OUTCOME_MULTIPLIER_AUDIT, 1.0)
            loss = ((raw * mult) ** NONLINEAR_POWER_AUDIT) * weights
            return {
        ...
        search_dirs = []
        if "SUB_DIR" in globals():
            search_dirs.append(Path(SUB_DIR))
        if "OUT_DIR" in globals():
            search_dirs.append(Path(OUT_DIR) / "submissions")
        if "OUTPUT_ROOT" in globals():
            search_dirs += [
                Path(OUTPUT_ROOT) / "exp12e_pair_repair_micro_audit",
                Path(OUTPUT_ROOT) / "exp12d_exp22_alignment_hybrid",
                Path(OUTPUT_ROOT) / "exp12c_exp22_strength_rebase_hybrid",
                Path(OUTPUT_ROOT) / "exp12b_no_pseudo_pair_native_decoder_alignment",
            ]
        search_dirs = [p for p in search_dirs if Path(p).exists()]
        ...
        for path in submission_paths:
            try:
                ...
                required_sub_cols = ["Id", "team_goals", "opp_goals"]
                missing = [c for c in required_sub_cols if c not in sub.columns]
                if missing:
                    bad_rows.append({"file_name": path.name, "path": str(path), "reason": f"missing {missing}"})
                    continue
                sub = sub[required_sub_cols].copy()
                ...
                metric.update({"file_name": path.name, "path": str(path), "n_rows": len(merged)})
                rows.append(metric)
            except Exception as e:
        ...
        display(local_gt_audit_rank_df.head(40))

        audit_dir = EXP12E_SUM_DIR if "EXP12E_SUM_DIR" in globals() else (PROJECT_ROOT / "outputs" / "local_gt_audit")
        audit_dir = Path(audit_dir)
        audit_dir.mkdir(parents=True, exist_ok=True)
        local_gt_audit_rank_df.to_csv(audit_dir / "local_gt_audit_awmae_rank.csv", index=False)
        local_gt_audit_rank_df.to_csv(audit_dir / "local_gt_audit_awmae.csv", index=False)
        audit_log(f"Saved {audit_dir / 'local_gt_audit_awmae_rank.csv'}")

        if bad_rows:
            bad_df = pd.DataFrame(bad_rows)
            display(bad_df.head(40))
            bad_df.to_csv(audit_dir / "local_gt_audit_bad_files.csv", index=False)
            audit_log(f"Saved {audit_dir / 'local_gt_audit_bad_files.csv'}")
