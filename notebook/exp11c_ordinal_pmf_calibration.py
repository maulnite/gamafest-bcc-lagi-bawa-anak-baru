# %% [markdown]
# # 00. EXP11C — Ringkasan Eksperimen
#
...
# XGBoost/TabPFN/tqdm, dan output disimpan ke folder EXP11C.
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'exp11c_ordinal_pmf_calibration'
...
# # 21. Final Decision
#
# Bagian ini memilih calibration terbaik berdasarkan AW-MAE dan safety check. Jika calibration
# tidak aman atau gain terlalu kecil dengan perubahan prediksi besar, eksperimen fallback ke
# C0 EXP11A original. Final decision disimpan sebagai JSON dan ditampilkan sebagai DataFrame.

# %%
log_section('Final decision for EXP11C')
metrics = pmf_calibration_metrics_df.copy()
c0 = metrics[metrics['variant_name'].eq('C0_exp11a_original')].iloc[0]
best_row = metrics.sort_values('awmae').iloc[0]

safe_candidates = []
for _, row in metrics[~metrics['variant_name'].eq('C0_exp11a_original')].iterrows():
    gain = float(c0['awmae']) - float(row['awmae'])
    safe = (
        (gain > 0.0)
        and (float(row['base_mae']) <= float(c0['base_mae']) + 0.002)
        and (float(row['exact_rate']) >= float(c0['exact_rate']) - 0.006)
        and (float(row['outcome_rate']) >= float(c0['outcome_rate']) - 0.004)
        and (float(row['gd_rate']) >= float(c0['gd_rate']) - 0.006)
        and (float(row['top3_scoreline_share']) <= float(c0['top3_scoreline_share']) + 0.030)
        and (1.0 <= float(row['mean_pred_total']) <= 4.5)
        and (float(row['total_goal_bias']) >= float(c0['total_goal_bias']) - 0.030)
        and (abs(float(row['pmf_mean_total']) - float(c0['pmf_mean_total'])) <= 0.50)
        and not ((gain < 0.001) and (float(row.get('changed_rate_vs_c0', 0.0)) > 0.15))
    )
    if safe:
...
if safe_candidates:
    ...
    selected_calibration_name = str(selected_row['variant_name'])
    decision_code = 'A'
    decision_text = 'PMF calibration improves AW-MAE and passes safety checks.'
elif str(best_row['variant_name']) == 'C0_exp11a_original':
    selected_row = c0
    selected_calibration_name = 'C0_exp11a_original'
    decision_code = 'C'
    decision_text = 'EXP11A original remains best; PMF calibration is not selected.'
else:
    selected_row = c0
    selected_calibration_name = 'C0_exp11a_original'
    if (
        float(best_row.get('total_goal_bias', -999)) > float(c0.get('total_goal_bias', -999))
        or float(best_row.get('pmf_mean_total', 0)) > float(c0.get('pmf_mean_total', 0))
    ):
        decision_code = 'B'
        decision_text = 'Calibration improves PMF quality or bias but not safely enough on AW-MAE, fallback to EXP11A.'
    else:
        decision_code = 'D'
        decision_text = 'Best calibration by validation is unsafe, fallback to EXP11A original.'

selected_pmf_calibration_config = copy.deepcopy(CALIBRATION_RESULTS[selected_calibration_name]['config'])
selected_json = {
    'experiment': 'EXP11C',
    'base_pipeline': 'EXP05A-LITE-FIX-V2 / EXP09D / EXP11A',
    'selected_gate': selected_gate_name,
    'selected_policy': 'M calibrated ordinal MBR with W fallback to EXP09D',
    'selected_calibration_variant': selected_calibration_name,
    'selected_pmf_calibration_config': selected_pmf_calibration_config,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'c0_awmae': float(c0['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_c0': float(float(c0['awmae']) - float(selected_row['awmae'])),
}
with open(SUM_DIR / 'selected_pmf_calibration_config.json', 'w', encoding='utf-8') as f:
    json.dump(selected_json, f, indent=2, ensure_ascii=False)

final_decision = {
    'experiment': 'EXP11C',
    'main_change': 'PMF calibration before EXP11A M-only ordinal MBR decoder',
    'selected_gate': selected_gate_name,
    'selected_calibration_variant': selected_calibration_name,
    'decision_code': decision_code,
    'decision_text': decision_text,
    ...
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_c0': float(float(c0['awmae']) - float(selected_row['awmae'])),
    'caveat': 'Training, feature engineering, ordinal target, PMF construction, and tournament gate are preserved. Only PMF calibration changes.',
}
...
selected_pmf_calibration_config_df = pd.DataFrame([{'key': k, 'value': str(v)} for k, v in selected_json.items()])
final_decision_df.to_csv(SUM_DIR / 'final_decision_table.csv', index=False)
CALIBRATION_RESULTS[selected_calibration_name]['pred_match'].to_csv(PRED_DIR / 'valid_pred_match_best_calibration.csv', index=False)
log_saved(PRED_DIR / 'valid_pred_match_best_calibration.csv')
display(metrics.sort_values('awmae').reset_index(drop=True))
display(selected_pmf_calibration_config_df)
display(final_decision_df)
log_saved(SUM_DIR / 'selected_pmf_calibration_config.json')
...
# menerapkan selected PMF calibration, lalu decode final. Jika C0 dipilih, output tetap sama secara
# konsep dengan EXP11A original: M ordinal MBR dan W EXP09D original.
...
test_pmf_cal_a = calibrate_goal_pmf(test_pmf_ord_a, test_pmf_pois_a, selected_pmf_calibration_config)
test_pmf_cal_b = calibrate_goal_pmf(test_pmf_ord_b, test_pmf_pois_b, selected_pmf_calibration_config)

test_pred = decode_exp11c_policy(test_raw_gate, test_pred_exp09d, test_pmf_cal_a, test_pmf_cal_b, selected_calibration_name)
...
test_pred_match_best['selected_calibration_variant'] = selected_calibration_name
test_pred_match_best.to_csv(PRED_DIR / 'test_pred_exp11c_best_safe.csv', index=False)
log_saved(PRED_DIR / 'test_pred_exp11c_best_safe.csv')

print('[TEST INFERENCE SUMMARY]', flush=True)
log_info(f'selected_gate: {selected_gate_name}')
log_info(f'selected_calibration_variant: {selected_calibration_name}')
...
sub_path = SUB_DIR / 'submission_exp11c_best_safe.csv'
...
# adalah selected calibration config, decision code, validation metric, dan path submission final.
...
summary_df = pd.DataFrame([
    {'item': 'experiment', 'value': 'EXP11C'},
    {'item': 'selected_gate', 'value': selected_gate_name},
    {'item': 'selected_calibration_variant', 'value': selected_calibration_name},
...
log_result('EXP11C completed. Check validation safety, PMF quality, domain tables, and pair consistency before submitting.')
