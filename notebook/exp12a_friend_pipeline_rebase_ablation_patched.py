# %% [markdown]
# # 00. EXP12A — Friend Pipeline Rebase dan Ablation Switchboard
#
# Bagian ini menjelaskan tujuan EXP12A. Eksperimen ini bukan lanjutan patch EXP11C,
# melainkan rebase pipeline teman `awmae_exp18_ablation_switchboard` sebagai backbone baru.
# Notebook/script ini mempertahankan ide utama pipeline teman: ablation switchboard,
# country-aware features, missing-data imputer, pseudo-labelling, extreme overlay,
# EXP15 ordinal distribution, dan EXP17 W-specialist overlay.
#
# Output yang diharapkan:
# - submission per ablation variant yang dijalankan,
# - summary validation/sanity check,
# - artifact diagnostic untuk pseudo-label, extreme overlay, EXP15, dan EXP17.

# %% [markdown]
# # 01. Setup, Seed, Path, dan Guardrail
#
# Bagian ini menyiapkan library, seed, path input/output, konfigurasi switchboard,
# dan guardrail legal. Output yang diharapkan adalah tabel konfigurasi ablation
# yang sedang dijalankan dan path output yang jelas.

# %%
# ============================================================
# 01. Setup, Seed, Path, Guardrail, dan EXP12A Switchboard
# ============================================================
...
# EXP12A switchboard
# ------------------------------
# Jalankan satu variant per run. Untuk menjalankan variant lain, ganti nilai ini
# atau set environment variable EXP12A_ABLATION_VARIANT sebelum run script.
EXP12A_ABLATION_VARIANT = os.environ.get("EXP12A_ABLATION_VARIANT", "a10_full_reproduction").strip()

# Smoke test dapat dipakai untuk cek end-to-end sampai submission.
FAST_MODE = bool(int(os.environ.get("EXP12A_FAST_MODE", "0")))
...
# Friend-pipeline ablation config. Ini memakai flag yang sama dengan source notebook teman.
EXP12A_ABLATION_CONFIGS = {
    "a0_minimal_safe": {
...
if EXP12A_ABLATION_VARIANT not in EXP12A_ABLATION_CONFIGS:
    raise ValueError(
        f"Unknown EXP12A_ABLATION_VARIANT={EXP12A_ABLATION_VARIANT}. "
        f"Pilih salah satu: {list(EXP12A_ABLATION_CONFIGS)}"
    )

for _k, _v in EXP12A_ABLATION_CONFIGS[EXP12A_ABLATION_VARIANT].items():
    globals()[_k] = _v
...
EXP18_ABLATION_VARIANT = EXP12A_ABLATION_VARIANT

OUTPUT_ROOT = PROJECT_ROOT / "outputs" / "exp12a_friend_pipeline_rebase_ablation"
OUT_DIR = OUTPUT_ROOT / EXP12A_ABLATION_VARIANT
...
# Source notebook memakai OUT_DIR langsung. Kita tetap expose OUT_DIR dan menyimpan
# tambahan artifact terstruktur ke PRED_DIR/SUB_DIR/SUM_DIR di akhir.
ABLATION_ROOT_DIR = OUTPUT_ROOT

log_section("EXP12A setup")
log_info(f"EXP12A_ABLATION_VARIANT: {EXP12A_ABLATION_VARIANT}")
...
with open(SUM_DIR / "ablation_config.json", "w", encoding="utf-8") as f:
    json.dump(EXP12A_ABLATION_CONFIGS[EXP12A_ABLATION_VARIANT], f, indent=2)
...
# %% [markdown]
# # 18. EXP12A Submission Export, Pair Repair, Sanity Check, dan Final Decision
#
# Bagian ini mengekspor semua submission variant yang tersedia ke folder EXP12A.
# Beberapa branch dari pipeline teman bisa menghasilkan prediksi row-level yang belum mirror-consistent.
# Karena kompetisi menyimpan satu match sebagai dua row, bagian ini melakukan align ke sample submission,
# memperbaiki pair consistency secara legal dari dua row mirror yang sama, lalu menjalankan sanity check final.
# Output pentingnya adalah katalog submission, tabel submission check, repair log, dan final decision.

# %%
# ============================================================
# 18. EXP12A submission export, pair repair, sanity check, and final decision
# ============================================================
...
def align_submission_to_sample(sub_df: pd.DataFrame) -> pd.DataFrame:
    """
    Align arbitrary row-level submission to sample order.
    This does not change predictions except row ordering and id-column normalization.
    """
    sub = normalize_submission_columns(sub_df)

    sample_ids = sample_submission[sample_id_col].astype(str).tolist()
    sample_aligned = sample_submission[[sample_id_col]].copy().rename(columns={sample_id_col: ID_COL})
    sample_aligned[ID_COL] = sample_aligned[ID_COL].astype(str)
    ...
    if aligned[["team_goals", "opp_goals"]].isna().any().any():
        missing_ids = aligned.loc[
            aligned[["team_goals", "opp_goals"]].isna().any(axis=1),
            ID_COL,
        ].head(10).tolist()
        raise RuntimeError(f"Aligned submission has missing predictions. Examples: {missing_ids}")

    aligned["team_goals"] = pd.to_numeric(aligned["team_goals"], errors="raise").round().astype(int).clip(lower=0)
    aligned["opp_goals"] = pd.to_numeric(aligned["opp_goals"], errors="raise").round().astype(int).clip(lower=0)

    assert aligned[ID_COL].astype(str).tolist() == sample_ids, "Aligned submission id order differs from sample"
    return aligned
...
def make_pair_consistent_submission(
    aligned_sub_df: pd.DataFrame,
    row_df: pd.DataFrame,
    label: str,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Enforce mirror consistency for every two-row match.

    If row0 predicts (a0,b0) and row1 predicts (a1,b1), a consistent match prediction can be
    reconstructed by averaging the two mirrored estimates:
      row0 team goals estimate = mean(a0, b1)
      row0 opp goals estimate  = mean(b0, a1)
    Then row1 is assigned the mirror score.

    This repair uses only model predictions and test match_id structure. It does not use any GT.
    """
    required_row_cols = [ID_COL, MATCH_COL]
    missing = [c for c in required_row_cols if c not in row_df.columns]
    if missing:
        raise KeyError(f"row_df missing required columns for pair repair: {missing}")

    sub = align_submission_to_sample(aligned_sub_df)

    row_map = row_df[[ID_COL, MATCH_COL]].copy()
    row_map[ID_COL] = row_map[ID_COL].astype(str)
    row_map["_id_key"] = row_map[ID_COL]
    row_map["_row_order"] = np.arange(len(row_map))

    work = sub.copy()
    work["_id_key"] = work[ID_COL].astype(str)
    work = work.merge(
        row_map[["_id_key", MATCH_COL, "_row_order"]],
        on="_id_key",
        how="left",
        validate="one_to_one",
    )

    if work[MATCH_COL].isna().any():
        bad = work.loc[work[MATCH_COL].isna(), ID_COL].head(10).tolist()
        raise RuntimeError(f"Cannot map some submission IDs to match_id. Examples: {bad}")
    ...
    for match_id, g in work.groupby(MATCH_COL, sort=False):
        ...
        if len(g) != 2:
            raise RuntimeError(f"Expected exactly two rows for match_id={match_id}, got {len(g)}")

        idx0, idx1 = g.index[0], g.index[1]
        r0, r1 = g.loc[idx0], g.loc[idx1]

        a_raw = int(r0["team_goals"])
        b_raw = int(r0["opp_goals"])
        a_mirror = int(r1["opp_goals"])
        b_mirror = int(r1["team_goals"])

        a_new = max(0, _round_half_up((a_raw + a_mirror) / 2.0))
        b_new = max(0, _round_half_up((b_raw + b_mirror) / 2.0))

        changed = (
            int(r0["team_goals"]) != a_new
            or int(r0["opp_goals"]) != b_new
            or int(r1["team_goals"]) != b_new
            or int(r1["opp_goals"]) != a_new
        )

        repaired.loc[idx0, "team_goals"] = a_new
        repaired.loc[idx0, "opp_goals"] = b_new
        repaired.loc[idx1, "team_goals"] = b_new
        repaired.loc[idx1, "opp_goals"] = a_new

        repair_rows.append({
            "variant": label,
            "match_id": match_id,
            "changed": bool(changed),
            "row0_id": str(r0[ID_COL]),
            "row1_id": str(r1[ID_COL]),
            "row0_before": f"{int(r0['team_goals'])}-{int(r0['opp_goals'])}",
            "row1_before": f"{int(r1['team_goals'])}-{int(r1['opp_goals'])}",
            "row0_after": f"{a_new}-{b_new}",
            "row1_after": f"{b_new}-{a_new}",
        })

    repaired = repaired.sort_values("_row_order")
    repaired = repaired[[ID_COL, "team_goals", "opp_goals"]].copy()
    repaired[ID_COL] = repaired[ID_COL].astype(str)

    # Restore exact sample order.
    repaired = align_submission_to_sample(repaired)
    repair_log = pd.DataFrame(repair_rows)
    return repaired, repair_log


def check_submission_pair_consistency(sub_df: pd.DataFrame, row_df: pd.DataFrame) -> bool:
    try:
        repaired, repair_log = make_pair_consistent_submission(sub_df, row_df, label="check_only")
        return not bool(repair_log["changed"].any())
    except Exception:
        return False


def validate_and_save_exp12a_submission(sub_df: pd.DataFrame, label: str) -> tuple[pd.DataFrame, Path, pd.DataFrame]:
    aligned_raw = align_submission_to_sample(sub_df)
    was_pair_consistent_before = check_submission_pair_consistency(aligned_raw, test_raw)

    aligned, repair_log = make_pair_consistent_submission(aligned_raw, test_raw, label=label)
    n_repaired_matches = int(repair_log["changed"].sum()) if len(repair_log) else 0
    repair_rate = float(repair_log["changed"].mean()) if len(repair_log) else 0.0

    repair_log_path = SUM_DIR / f"pair_consistency_repair_{label}.csv"
    repair_log.to_csv(repair_log_path, index=False)
    log_saved(repair_log_path)

    if n_repaired_matches > 0:
        log_info(
            f"{label}: repaired {n_repaired_matches:,}/{len(repair_log):,} match pairs "
            f"for mirror consistency ({repair_rate:.2%})."
        )
    ...
    checks = [
        ...
        {"variant": label, "check": "pair_consistency_before_repair", "passed": bool(was_pair_consistent_before)},
        {"variant": label, "check": "pair_consistency_after_repair", "passed": bool(check_submission_pair_consistency(aligned, test_raw))},
        {"variant": label, "check": "max_goal_sanity_le_40", "passed": bool((aligned[["team_goals", "opp_goals"]] <= 40).all().all())},
    ]
    check_df = pd.DataFrame(checks)

    path = SUB_DIR / f"submission_exp12a_{label}.csv"
    aligned.to_csv(path, index=False)
    log_saved(path)

    # Only final, post-repair consistency is fatal. The before-repair check is diagnostic.
    fatal_checks = check_df.loc[check_df["check"] != "pair_consistency_before_repair", "passed"]
    if not bool(fatal_checks.all()):
        display(check_df)
...
# Base advanced pipeline output always exists after Section 13/14.
if "submission" in globals():
    sub_base, path_base, check_base = validate_and_save_exp12a_submission(submission, f"{EXP12A_ABLATION_VARIANT}_advanced_base")
    submission_catalog.append({"label": f"{EXP12A_ABLATION_VARIANT}_advanced_base", "path": str(path_base), "source": "advanced_base"})
    submission_check_frames.append(check_base)

# EXP15 output if enabled and completed.
if "submission_exp15" in globals():
    sub_exp15, path_exp15, check_exp15 = validate_and_save_exp12a_submission(submission_exp15, f"{EXP12A_ABLATION_VARIANT}_exp15_ordinal")
    submission_catalog.append({"label": f"{EXP12A_ABLATION_VARIANT}_exp15_ordinal", "path": str(path_exp15), "source": "exp15"})
    submission_check_frames.append(check_exp15)

# EXP17 output if enabled and completed.
if "submission_exp17" in globals():
    sub_exp17, path_exp17, check_exp17 = validate_and_save_exp12a_submission(submission_exp17, f"{EXP12A_ABLATION_VARIANT}_exp17_w_specialist")
    submission_catalog.append({"label": f"{EXP12A_ABLATION_VARIANT}_exp17_w_specialist", "path": str(path_exp17), "source": "exp17"})
    submission_check_frames.append(check_exp17)

if "submission_exp17" in globals():
    primary_label = EXP12A_ABLATION_VARIANT
    primary_sub = submission_exp17
    selected_source = "exp17_w_specialist"
elif "submission_exp15" in globals():
    primary_label = EXP12A_ABLATION_VARIANT
    primary_sub = submission_exp15
    selected_source = "exp15_ordinal"
elif "submission" in globals():
    primary_label = EXP12A_ABLATION_VARIANT
    primary_sub = submission
    selected_source = "advanced_base"
else:
    raise RuntimeError("No submission object found. Pipeline did not reach final prediction stage.")

primary_submission, primary_path, primary_check = validate_and_save_exp12a_submission(primary_sub, primary_label)
submission_catalog.append({"label": primary_label, "path": str(primary_path), "source": selected_source})
...
# Diagnostics required when reproduction is far from target are prepared structurally here.
diagnostics = {
    "experiment": "EXP12A",
    "variant": EXP12A_ABLATION_VARIANT,
    "selected_source": selected_source,
    "exp11c_reference_awmae": 3.052687,
    "best_so_far_hubert_reference_awmae": 3.005946,
    "exp15_enabled": bool(EXP15_ENABLED),
    "exp17_enabled": bool(EXP17_ENABLED),
    "country_features_enabled": bool(USE_COUNTRY_FEATURES),
    "missing_imputer_enabled": bool(USE_MISSING_IMPUTER),
    "extreme_clusters_enabled": bool(USE_EXTREME_CLUSTERS),
    "pseudo_labelling_enabled": bool(USE_PSEUDO_LABELLING),
    "extreme_overlay_enabled": bool(USE_EXTREME_OVERLAY),
    "primary_submission_path": str(primary_path),
    "all_submission_paths": submission_catalog_df["path"].tolist(),
}
...
final_decision = {
    ...
    "main_candidate_submission": str(primary_path),
    "note": (
        "EXP12A is a rebase of the friend EXP18/EXP15/EXP17 switchboard. "
        "GT audit is disabled by default and must not be used for training/tuning. "
        "Any row-level non-mirror predictions are repaired using test match_id structure only."
    ),
}

with open(SUM_DIR / "final_decision.json", "w", encoding="utf-8") as f:
    json.dump(final_decision, f, indent=2)
with open(SUM_DIR / "reproduction_diagnostics.json", "w", encoding="utf-8") as f:
...
log_info(f"variant: {EXP12A_ABLATION_VARIANT}")
...
display(top_scores)
top_scores.to_csv(SUM_DIR / "scoreline_distribution_top15.csv", index=False)
log_saved(SUM_DIR / "scoreline_distribution_top15.csv")


# %% [markdown]
# # 19. Runtime Summary dan Catatan Akhir
#
# Bagian ini menutup run EXP12A. Script ini adalah switchboard: untuk menjalankan variant lain,
# ubah `EXP12A_ABLATION_VARIANT` atau environment variable dengan nama yang sama, lalu run ulang.
# Prioritas audit lokal biasanya: `a8_exp15_ordinal`, `a9_exp17_w_specialist`, lalu `a10_full_reproduction`.

# %%
print("EXP12A selesai ✓", flush=True)
print("Variant:", EXP12A_ABLATION_VARIANT, flush=True)
