# %% [markdown]
# # 00. EXP12B — No-Pseudo Pair-Native Decoder Alignment
#
# Bagian ini menjelaskan tujuan EXP12B. Eksperimen ini adalah refinement langsung dari EXP12B,
# bukan eksperimen baru dari nol. Core friend pipeline tetap dipreserve: country-aware features,
# missing imputer, EXP15 ordinal PMF, dan EXP17 W-specialist overlay.
#
# Fokus utama EXP12B:
# - no-pseudo ablation untuk EXP15/EXP17,
# - native pair-consistent decode/export path,
# - decoder calibration kecil yang terstruktur untuk EXP15/EXP17,
# - semua submission variant disimpan agar bisa diaudit lokal.

# %% [markdown]
# # 01. Setup, Seed, Path, dan Guardrail
#
# Bagian ini menyiapkan library, seed, path input/output, konfigurasi switchboard EXP12B,
# dan guardrail legal. Output yang diharapkan adalah tabel konfigurasi variant yang sedang dijalankan,
# path output yang jelas, serta flag no-pseudo / pair-native / calibration yang eksplisit.

# %%
# ============================================================
# 01. Setup, Seed, Path, Guardrail, dan EXP12B Switchboard
# ============================================================
...
# ------------------------------
# Legal / anti-leakage guardrail
# ------------------------------
USE_GT_FOR_TRAINING = False
USE_GT_FOR_TUNING = False
USE_GT_FOR_PSEUDO_LABEL_SELECTION = False
USE_EXTERNAL_CSV = False
USE_SUBMISSION_AS_FEATURE = False
USE_OLD_PREDICTION_AS_FEATURE = False
USE_TABPFN = False
USE_XGBOOST = False
USE_TQDM = False

RUN_GT_AUDIT = False
RUN_EXP15_GT_AUDIT = False
RUN_EXP17_GT_AUDIT = False

for _name, _value in {
    "GT training disabled": not USE_GT_FOR_TRAINING,
    "GT tuning disabled": not USE_GT_FOR_TUNING,
    "GT pseudo-label selection disabled": not USE_GT_FOR_PSEUDO_LABEL_SELECTION,
    "External CSV disabled": not USE_EXTERNAL_CSV,
    "Old submission as feature disabled": not USE_SUBMISSION_AS_FEATURE,
    "Old prediction as feature disabled": not USE_OLD_PREDICTION_AS_FEATURE,
    "TabPFN disabled": not USE_TABPFN,
    "XGBoost disabled": not USE_XGBOOST,
    "tqdm disabled": not USE_TQDM,
}.items():
    log_check(_name, _value)
...
def find_file_any(filenames):
    ...
    for d in CANDIDATE_DIRS:
        for filename in filenames:
            ...
            if p.exists():
                return p
    raise FileNotFoundError(f"Tidak menemukan salah satu file: {filenames}. Cari di: {CANDIDATE_DIRS}")

TRAIN_PATH = find_file_any(["train.csv"])
TEST_PATH = find_file_any(["test.csv"])
SAMPLE_PATH = find_file_any(["sample_submission.csv", "sample submission.csv"])
METADATA_PATH = None
...
# EXP12B switchboard
# ------------------------------
# Jalankan satu variant per run. Ganti nilai ini atau set environment variable EXP12B_VARIANT.
EXP12B_VARIANT = os.environ.get("EXP12B_VARIANT", "b5_exp15_calibrated").strip()

# Smoke test dapat dipakai untuk cek end-to-end sampai submission.
FAST_MODE = bool(int(os.environ.get("EXP12B_FAST_MODE", "0")))
...
MODEL_TRAIN_START_DATE = "1990-01-01"
...
# EXP15/EXP17 runtime controls. FAST_MODE lebih ringan untuk smoke test.
...
# EXP12B variant config. B1-B6 adalah no-pseudo mainline; B0 hanya reference EXP12B.
EXP12B_VARIANT_CONFIGS = {
    "b0_exp12a_reference": {
        ...
        "EXP15_ENABLED": True,
        "EXP17_ENABLED": True,
        "PAIR_NATIVE_DECODE": False,
        "DECODER_CALIBRATION": False,
        "EXP17_W_CALIBRATION": False,
    },
    "b1_exp15_no_pseudo": {
        "USE_COUNTRY_FEATURES": True,
        "USE_MISSING_IMPUTER": True,
    ...
    "b3_exp15_no_pseudo_pair_native": {
        "USE_COUNTRY_FEATURES": True,
        "USE_MISSING_IMPUTER": True,
        "USE_EXTREME_CLUSTERS": True,
        "USE_PSEUDO_LABELLING": False,
        ...
        "EXP15_ENABLED": True,
        "EXP17_ENABLED": False,
        "PAIR_NATIVE_DECODE": True,
        "DECODER_CALIBRATION": False,
        "EXP17_W_CALIBRATION": False,
    },
    "b4_exp17_no_pseudo_pair_native": {
        "USE_COUNTRY_FEATURES": True,
        "USE_MISSING_IMPUTER": True,
        "USE_EXTREME_CLUSTERS": True,
        "USE_PSEUDO_LABELLING": False,
    ...
    "b5_exp15_calibrated": {
        ...
        "EXP17_ENABLED": False,
        "PAIR_NATIVE_DECODE": True,
        "DECODER_CALIBRATION": True,
        "EXP17_W_CALIBRATION": False,
    },
    "b6_exp17_w_calibrated": {
        "USE_COUNTRY_FEATURES": True,
        "USE_MISSING_IMPUTER": True,
        "USE_EXTREME_CLUSTERS": True,
        "USE_PSEUDO_LABELLING": False,
        ...
        "PAIR_NATIVE_DECODE": True,
        "DECODER_CALIBRATION": True,
        "EXP17_W_CALIBRATION": True,
    },
}

if EXP12B_VARIANT not in EXP12B_VARIANT_CONFIGS:
    raise ValueError(
        f"Unknown EXP12B_VARIANT={EXP12B_VARIANT}. "
        f"Pilih salah satu: {list(EXP12B_VARIANT_CONFIGS)}"
    )

for _k, _v in EXP12B_VARIANT_CONFIGS[EXP12B_VARIANT].items():
    globals()[_k] = _v

# Safety dependency: extreme overlay butuh country-prior + cluster.
USE_EXTREME_OVERLAY = bool(USE_EXTREME_OVERLAY and USE_COUNTRY_FEATURES and USE_EXTREME_CLUSTERS)

# Legacy alias supaya code dari notebook teman tetap minim perubahan.
EXP18_ABLATION_VARIANT = EXP12B_VARIANT
EXP12B_VARIANT = EXP12B_VARIANT

OUTPUT_ROOT = PROJECT_ROOT / "outputs" / "exp12b_no_pseudo_pair_native_decoder_alignment"
OUT_DIR = OUTPUT_ROOT / EXP12B_VARIANT
FIG_DIR = OUT_DIR / "figures"
PRED_DIR = OUT_DIR / "predictions"
SUB_DIR = OUT_DIR / "submissions"
SUM_DIR = OUT_DIR / "summaries"

for _d in [OUTPUT_ROOT, OUT_DIR, FIG_DIR, PRED_DIR, SUB_DIR, SUM_DIR]:
    _d.mkdir(parents=True, exist_ok=True)

ABLATION_ROOT_DIR = OUTPUT_ROOT

log_section("EXP12B setup")
log_info(f"EXP12B_VARIANT: {EXP12B_VARIANT}")
log_info(f"FAST_MODE: {FAST_MODE}")
log_info(f"TRAIN_PATH: {TRAIN_PATH.resolve()}")
log_info(f"TEST_PATH : {TEST_PATH.resolve()}")
log_info(f"SAMPLE_PATH: {SAMPLE_PATH.resolve()}")
log_info(f"OUT_DIR   : {OUT_DIR.resolve()}")

ablation_config_df = pd.DataFrame([
    {"key": k, "value": str(globals().get(k))}
    for k in [
        "USE_COUNTRY_FEATURES",
        "USE_MISSING_IMPUTER",
        "USE_EXTREME_CLUSTERS",
        "USE_PSEUDO_LABELLING",
        "USE_EXTREME_OVERLAY",
        "EXP15_ENABLED",
        "EXP17_ENABLED",
        "PAIR_NATIVE_DECODE",
        ...
        "EXP15_MAX_PRED_GOAL",
        "EXP17_W_MAX_GOAL",
        "EXP17_W_MAX_PRED_GOAL",
    ]
])
display(ablation_config_df)
ablation_config_df.to_csv(SUM_DIR / "ablation_config.csv", index=False)
with open(SUM_DIR / "ablation_config.json", "w", encoding="utf-8") as f:
    json.dump(EXP12B_VARIANT_CONFIGS[EXP12B_VARIANT], f, indent=2)

log_saved(SUM_DIR / "ablation_config.csv")
...
EXP15_ENABLED = bool(EXP15_ENABLED)
...
EXP15_MAX_GOAL = int(EXP15_MAX_GOAL)

# Kandidat scoreline maksimum saat decoder memilih prediksi final.
EXP15_MAX_PRED_GOAL = int(EXP15_MAX_PRED_GOAL)

# Runtime control.
EXP15_FAST_MODE = bool(FAST_MODE)
EXP15_USE_GENDER_SEGMENT = True
EXP15_MIN_SEGMENT_ROWS = 1800
...
EXP17_ENABLED = bool(EXP17_ENABLED)
...
EXP17_W_MAX_GOAL = int(EXP17_W_MAX_GOAL)
EXP17_W_MAX_PRED_GOAL = int(EXP17_W_MAX_PRED_GOAL)
EXP17_W_MIN_TRAIN_ROWS = 700
EXP17_W_MIN_VALID_ROWS = 100
...
# %% [markdown]
# # 17B. Native Pair Decode dan Decoder Calibration Diagnostics
#
# Bagian ini menambahkan layer EXP12B di atas output row-level EXP12B. Tujuannya memastikan
# variant native pair menghasilkan submission yang konsisten pada level match sebelum validasi akhir.
# Decoder calibration disimpan sebagai grid diagnostic terstruktur; core EXP15/EXP17 tetap berasal
# dari pipeline teman yang sudah dipreserve.

# %%
# ============================================================
# 17B. Native pair helper + EXP12B diagnostic config
# ============================================================

def exp12b_native_pair_consistent_submission(row_prediction: pd.DataFrame, row_df: pd.DataFrame, label: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Build pair-consistent row-level submission by aggregating each mirrored match once.
    This is a native match-level export path for EXP12B variants. It uses only model
    predictions and test match structure, not GT.
    """
    pred = row_prediction.copy()
    if ID_COL not in pred.columns and "id" in pred.columns:
        pred = pred.rename(columns={"id": ID_COL})
    pred = pred[[ID_COL, "team_goals", "opp_goals"]].copy()
    pred["team_goals"] = pd.to_numeric(pred["team_goals"], errors="raise").round().astype(int).clip(lower=0)
    pred["opp_goals"] = pd.to_numeric(pred["opp_goals"], errors="raise").round().astype(int).clip(lower=0)

    ids = row_df[[ID_COL, MATCH_COL]].copy()
    ids["_id_key"] = ids[ID_COL].astype(str)
    pred["_id_key"] = pred[ID_COL].astype(str)
    merged = ids.merge(pred[["_id_key", "team_goals", "opp_goals"]], on="_id_key", how="left", validate="one_to_one")

    if merged[["team_goals", "opp_goals"]].isna().any().any():
        bad = merged.loc[merged[["team_goals", "opp_goals"]].isna().any(axis=1), ID_COL].head().tolist()
        raise RuntimeError(f"Native pair decode missing predictions for id examples: {bad}")
    ...
    for match_id, g in merged.groupby(MATCH_COL, sort=False):
        if len(g) != 2:
            for _, r in g.iterrows():
                records.append({ID_COL: r[ID_COL], "team_goals": int(r["team_goals"]), "opp_goals": int(r["opp_goals"])})
            report_rows.append({"variant": label, "match_id": match_id, "n_rows": len(g), "native_pair_ok": False, "changed_by_native": False})
            continue

        r0 = g.iloc[0]
        r1 = g.iloc[1]
        a = int(round((float(r0["team_goals"]) + float(r1["opp_goals"])) / 2.0))
        b = int(round((float(r0["opp_goals"]) + float(r1["team_goals"])) / 2.0))
        a = max(0, a)
        b = max(0, b)
        changed = (
            int(r0["team_goals"]) != a or int(r0["opp_goals"]) != b or
            int(r1["team_goals"]) != b or int(r1["opp_goals"]) != a
        )
        records.append({ID_COL: r0[ID_COL], "team_goals": a, "opp_goals": b})
        records.append({ID_COL: r1[ID_COL], "team_goals": b, "opp_goals": a})
        report_rows.append({"variant": label, "match_id": match_id, "n_rows": 2, "native_pair_ok": True, "changed_by_native": bool(changed)})

    out = pd.DataFrame(records).drop_duplicates(subset=[ID_COL], keep="first")
    order = row_df[[ID_COL]].copy()
    order["_id_key"] = order[ID_COL].astype(str)
    out["_id_key"] = out[ID_COL].astype(str)
    out = order.merge(out[["_id_key", "team_goals", "opp_goals"]], on="_id_key", how="left", validate="one_to_one").drop(columns=["_id_key"])
    out = out[[ID_COL, "team_goals", "opp_goals"]].copy()
    ...
    report["pair_repair_count"] = 0
    return out, report


def exp12b_select_primary_submission() -> tuple[str, pd.DataFrame, str]:
    if bool(globals().get("EXP17_ENABLED", False)) and "submission_exp17" in globals():
        return "exp17_w_specialist", submission_exp17, "exp17"
    if bool(globals().get("EXP15_ENABLED", False)) and "submission_exp15" in globals():
        return "exp15_ordinal", submission_exp15, "exp15"
    if "submission" in globals():
        return "advanced_base", submission, "advanced_base"
    raise RuntimeError("No submission object found for EXP12B export.")

EXP12B_DECODER_STAGE1_GRID = pd.DataFrame([
    {"stage": 1, "source_policy": sp, "blend_ordinal": bo, "max_pred_goal": mpg, "scale": 1.00, "offset": 0.00, "lambda_gd": 0.00, "lambda_total": 0.00, "lambda_outcome": 0.00}
    for sp in ["sym_avg", "raw", "higher_total", "second_raw"]
    for bo in [0.75, 0.85, 0.90, 0.95]
    for mpg in ([10] if FAST_MODE else [10, 12, 14])
])
EXP12B_DECODER_STAGE2_GRID = pd.DataFrame([
    {"stage": 2, "scale": sc, "offset": off}
    for sc in [0.97, 1.00, 1.03, 1.05]
    for off in [0.00, 0.03, 0.05, 0.08]
])
EXP12B_DECODER_STAGE3_GRID = pd.DataFrame([
    {"stage": 3, "lambda_gd": lgd, "lambda_total": lt, "lambda_outcome": lo}
    for lgd in [0.00, 0.02]
    for lt in [0.00, 0.02]
    for lo in [0.00, 0.04]
])

if bool(globals().get("DECODER_CALIBRATION", False)):
    EXP12B_DECODER_STAGE1_GRID.to_csv(SUM_DIR / "exp15_decoder_stage1.csv", index=False)
    EXP12B_DECODER_STAGE2_GRID.to_csv(SUM_DIR / "exp15_decoder_stage2.csv", index=False)
    EXP12B_DECODER_STAGE3_GRID.to_csv(SUM_DIR / "exp15_decoder_stage3.csv", index=False)
    decoder_selected_config = {
        "note": "EXP12B preserves EXP12B EXP15 decoder and stores staged calibration grids for audit. Full retuning can be enabled after smoke test.",
        "active_variant": EXP12B_VARIANT,
        "pair_native_decode": bool(PAIR_NATIVE_DECODE),
        "decoder_calibration": bool(DECODER_CALIBRATION),
    }
    with open(SUM_DIR / "exp15_decoder_selected_config.json", "w", encoding="utf-8") as f:
        json.dump(decoder_selected_config, f, indent=2)
    log_saved(SUM_DIR / "exp15_decoder_stage1.csv")
    log_saved(SUM_DIR / "exp15_decoder_stage2.csv")
    log_saved(SUM_DIR / "exp15_decoder_stage3.csv")
    log_saved(SUM_DIR / "exp15_decoder_selected_config.json")

# %% [markdown]
# # 17C. Patch EXP12B — True Native Pair Decode dan Real Staged Calibration
#
# Cell patch ini sengaja diletakkan setelah helper lama agar menimpa implementasi export-level averaging.
# Native pair decode di sini benar-benar membentuk PMF canonical match-level dari dua row mirror,
# lalu memilih satu scoreline dengan EXP15 MBR decoder sebelum dimapping kembali ke dua row submission.
# Jika DECODER_CALIBRATION=True, B5 juga menjalankan staged search sungguhan dan menyimpan hasilnya.

# %%
# ============================================================
# 17C. Override old native/export helpers with true native PMF decode
# ============================================================

def _exp12b_norm_pmf_row(p):
...
def _exp12b_pair_source(i, j, pmf_team, pmf_opp, mu_team, mu_opp, source_policy="sym_avg"):
    a_i = _exp12b_norm_pmf_row(pmf_team[i])      # row i team = canonical A
    a_j = _exp12b_norm_pmf_row(pmf_opp[j])       # row j opponent = canonical A
    b_i = _exp12b_norm_pmf_row(pmf_opp[i])       # row i opponent = canonical B
    b_j = _exp12b_norm_pmf_row(pmf_team[j])      # row j team = canonical B
    mu_a_i, mu_a_j = float(mu_team[i]), float(mu_opp[j])
    mu_b_i, mu_b_j = float(mu_opp[i]), float(mu_team[j])
    source_policy = str(source_policy or "sym_avg")

    if source_policy in {"raw", "first_raw"}:
        return a_i, b_i, mu_a_i, mu_b_i, "first_raw"
    if source_policy == "second_raw":
        return a_j, b_j, mu_a_j, mu_b_j, "second_raw"
    if source_policy == "higher_total":
        if (mu_a_i + mu_b_i) >= (mu_a_j + mu_b_j):
            return a_i, b_i, mu_a_i, mu_b_i, "higher_total_first"
        return a_j, b_j, mu_a_j, mu_b_j, "higher_total_second"
    if source_policy == "lower_total":
        if (mu_a_i + mu_b_i) <= (mu_a_j + mu_b_j):
            return a_i, b_i, mu_a_i, mu_b_i, "lower_total_first"
        return a_j, b_j, mu_a_j, mu_b_j, "lower_total_second"

    pmf_a = _exp12b_norm_pmf_row(0.5 * a_i + 0.5 * a_j)
    pmf_b = _exp12b_norm_pmf_row(0.5 * b_i + 0.5 * b_j)
    return pmf_a, pmf_b, 0.5 * (mu_a_i + mu_a_j), 0.5 * (mu_b_i + mu_b_j), "sym_avg"


def exp12b_build_native_match_inputs(row_df, pmf_raw, mu_team_raw, mu_opp_raw, source_policy="sym_avg"):
    df = row_df.reset_index(drop=True).copy()
    pmf_team = np.asarray(pmf_raw["team_goals"], dtype=float)
    pmf_opp = np.asarray(pmf_raw["opp_goals"], dtype=float)
    mu_team = np.asarray(mu_team_raw, dtype=float)
    mu_opp = np.asarray(mu_opp_raw, dtype=float)
    records, pmf_a_rows, pmf_b_rows, mu_a_rows, mu_b_rows = [], [], [], [], []
    bad_pairs = 0

    for match_id, idxs in df.groupby(MATCH_COL, sort=False).groups.items():
        idxs = list(idxs)
        if len(idxs) != 2:
            bad_pairs += 1
            continue
        i, j = idxs[0], idxs[1]
        pmf_a, pmf_b, mu_a, mu_b, selected_source = _exp12b_pair_source(
            i, j, pmf_team, pmf_opp, mu_team, mu_opp, source_policy=source_policy
        )
        records.append({
            MATCH_COL: match_id,
            "row_id_a": df.at[i, ID_COL],
            "row_id_b": df.at[j, ID_COL],
            "gender": df.at[i, "gender"] if "gender" in df.columns else "__NA__",
            "tournament": df.at[i, "tournament"] if "tournament" in df.columns else "__NA__",
            "source_policy": source_policy,
            "selected_source": selected_source,
        })
        pmf_a_rows.append(pmf_a); pmf_b_rows.append(pmf_b)
        mu_a_rows.append(mu_a); mu_b_rows.append(mu_b)

    if not records:
        raise RuntimeError("True native pair decode produced zero match rows.")
    return (
        pd.DataFrame(records),
        np.vstack(pmf_a_rows),
        np.vstack(pmf_b_rows),
        np.asarray(mu_a_rows, dtype=float),
        np.asarray(mu_b_rows, dtype=float),
        int(bad_pairs),
    )


def exp12b_match_pred_to_submission(match_pred_df, row_df):
    required = [MATCH_COL, "row_id_a", "row_id_b", "pred_team_a_goals", "pred_team_b_goals"]
    missing = [c for c in required if c not in match_pred_df.columns]
    if missing:
        raise KeyError(f"match_pred_df missing columns: {missing}")
    m = match_pred_df[required].copy()
    m["pred_team_a_goals"] = pd.to_numeric(m["pred_team_a_goals"], errors="raise").round().astype(int).clip(lower=0)
    m["pred_team_b_goals"] = pd.to_numeric(m["pred_team_b_goals"], errors="raise").round().astype(int).clip(lower=0)
    rows_a = pd.DataFrame({ID_COL: m["row_id_a"], "team_goals": m["pred_team_a_goals"], "opp_goals": m["pred_team_b_goals"]})
    rows_b = pd.DataFrame({ID_COL: m["row_id_b"], "team_goals": m["pred_team_b_goals"], "opp_goals": m["pred_team_a_goals"]})
    long = pd.concat([rows_a, rows_b], ignore_index=True)
    long["_id_key"] = long[ID_COL].astype(str)
    order = row_df[[ID_COL]].copy(); order["_id_key"] = order[ID_COL].astype(str)
    out = order.merge(long[["_id_key", "team_goals", "opp_goals"]], on="_id_key", how="left", validate="one_to_one").drop(columns="_id_key")
    if out[["team_goals", "opp_goals"]].isna().any().any():
        bad = out.loc[out[["team_goals", "opp_goals"]].isna().any(axis=1), ID_COL].head().tolist()
        raise RuntimeError(f"Native mapping missing row predictions for id examples: {bad}")
    out["team_goals"] = pd.to_numeric(out["team_goals"], errors="raise").astype(int).clip(lower=0)
    out["opp_goals"] = pd.to_numeric(out["opp_goals"], errors="raise").astype(int).clip(lower=0)
    return out[[ID_COL, "team_goals", "opp_goals"]]


def exp12b_decode_true_native_submission(row_df, pmf_raw, mu_team_raw, mu_opp_raw, config, label):
    cfg = dict(config)
    cfg.setdefault("decoder", "ordinal_mbr")
    cfg.setdefault("max_true", EXP15_MAX_GOAL)
    cfg.setdefault("max_pred", EXP15_MAX_PRED_GOAL)
    source_policy = cfg.get("source_policy", "sym_avg")
    match_df, pmf_a, pmf_b, mu_a, mu_b, bad_pairs = exp12b_build_native_match_inputs(
        row_df, pmf_raw, mu_team_raw, mu_opp_raw, source_policy=source_policy
    )
    pred_a, pred_b = exp15_decode_distribution(pmf_a, pmf_b, mu_a, mu_b, cfg, aux_pred=None)
    match_pred = match_df.copy()
    match_pred["pred_team_a_goals"] = pred_a.astype(int)
    match_pred["pred_team_b_goals"] = pred_b.astype(int)
    match_pred["variant"] = label
    sub = exp12b_match_pred_to_submission(match_pred, row_df)
    report = pd.DataFrame([{
        "variant": label,
        "native_decode_mode": "true_match_level_pmf_mbr",
        "source_policy": source_policy,
        "n_matches_decoded": int(len(match_pred)),
        "bad_input_pairs_before": int(bad_pairs),
        "pair_repair_count": 0,
        "native_pair_ok": bool(bad_pairs == 0),
    }])
    return sub, match_pred, report


def exp12b_metric_row(y_team, y_opp, pred_team, pred_opp, weights, label):
    y_team = np.asarray(y_team, dtype=int); y_opp = np.asarray(y_opp, dtype=int)
    pred_team = np.asarray(pred_team, dtype=int); pred_opp = np.asarray(pred_opp, dtype=int)
    vc = pd.Series(list(zip(pred_team, pred_opp))).value_counts(normalize=True)
    return {
        "variant": label,
        "awmae": float(awmae_score(y_team, y_opp, pred_team, pred_opp, weights=weights)),
        "base_mae": float((np.abs(y_team - pred_team) + np.abs(y_opp - pred_opp)).mean() / 2.0),
        "exact_rate": float(((y_team == pred_team) & (y_opp == pred_opp)).mean()),
        "outcome_rate": float((_outcome(y_team, y_opp) == _outcome(pred_team, pred_opp)).mean()),
        "gd_rate": float(((y_team - y_opp) == (pred_team - pred_opp)).mean()),
        "team_bias": float((pred_team - y_team).mean()),
        "opp_bias": float((pred_opp - y_opp).mean()),
        "total_goal_bias": float(((pred_team + pred_opp) - (y_team + y_opp)).mean()),
        "mean_pred_total": float((pred_team + pred_opp).mean()),
        "max_pred_goal": int(max(pred_team.max(), pred_opp.max())),
        "top1_scoreline_share": float(vc.iloc[0]) if len(vc) else 0.0,
        "top3_scoreline_share": float(vc.head(3).sum()) if len(vc) else 0.0,
    }


def exp12b_eval_native_config(config, label):
    valid_sub, valid_match, report = exp12b_decode_true_native_submission(
        valid_raw.reset_index(drop=True), exp15_valid_pmf_raw, valid_pred_team_raw, valid_pred_opp_raw, config, label
    )
    metric = exp12b_metric_row(valid_y_team, valid_y_opp, valid_sub["team_goals"].values, valid_sub["opp_goals"].values, valid_weights, label)
    for k, v in config.items():
        if isinstance(v, (str, int, float, bool)):
            metric[k] = v
    metric["native_bad_pairs_before"] = int(report["bad_input_pairs_before"].iloc[0])
    metric["pair_repair_count"] = 0
    return metric

# Staged calibration benar-benar dievaluasi pada validation, bukan hanya disimpan sebagai diagnostic.
EXP12B_STAGE1_GRID_TRUE = pd.DataFrame([
    {"stage": 1, "decoder": "ordinal_mbr", "source_policy": sp, "blend_ordinal": bo, "max_true": EXP15_MAX_GOAL, "max_pred": mpg, "scale": 1.00, "offset": 0.00, "lambda_gd": 0.00, "lambda_total": 0.00, "lambda_outcome": 0.00}
    for sp in ["sym_avg", "raw", "higher_total", "second_raw"]
    for bo in [0.75, 0.85, 0.90, 0.95]
    for mpg in ([10] if FAST_MODE else [10, 12, 14])
])
EXP12B_STAGE2_GRID_TRUE = pd.DataFrame([{"scale": sc, "offset": off} for sc in [0.97, 1.00, 1.03, 1.05] for off in [0.00, 0.03, 0.05, 0.08]])
EXP12B_STAGE3_GRID_TRUE = pd.DataFrame([{"lambda_gd": lgd, "lambda_total": lt, "lambda_outcome": lo} for lgd in [0.00, 0.02] for lt in [0.00, 0.02] for lo in [0.00, 0.04]])

EXP12B_NATIVE_SELECTED_CONFIG = dict(exp15_postprocessor.get("global", {})) if "exp15_postprocessor" in globals() else {"decoder": "ordinal_mbr", "source_policy": "sym_avg", "blend_ordinal": 0.85, "max_true": EXP15_MAX_GOAL, "max_pred": EXP15_MAX_PRED_GOAL, "scale": 1.0, "offset": 0.0, "lambda_gd": 0.0, "lambda_total": 0.0, "lambda_outcome": 0.0}

if bool(DECODER_CALIBRATION) and bool(EXP15_ENABLED):
    log_section("EXP12B true staged decoder calibration")
    stage1_df = pd.DataFrame([exp12b_eval_native_config(dict(r.dropna()), f"stage1_{i}") for i, r in EXP12B_STAGE1_GRID_TRUE.iterrows()]).sort_values("awmae").reset_index(drop=True)
    stage1_df.to_csv(SUM_DIR / "exp15_decoder_stage1.csv", index=False); log_saved(SUM_DIR / "exp15_decoder_stage1.csv"); display(stage1_df.head(10))

    stage2_rows = []
    for bi, base in enumerate(stage1_df.head(5 if not FAST_MODE else 3).to_dict("records")):
        for j, cal in EXP12B_STAGE2_GRID_TRUE.iterrows():
            cfg = {k: base[k] for k in ["decoder", "source_policy", "blend_ordinal", "max_true", "max_pred", "lambda_gd", "lambda_total", "lambda_outcome"] if k in base}
            cfg.update({"scale": float(cal["scale"]), "offset": float(cal["offset"])})
            stage2_rows.append(exp12b_eval_native_config(cfg, f"stage2_{bi}_{j}"))
    stage2_df = pd.DataFrame(stage2_rows).sort_values("awmae").reset_index(drop=True)
    stage2_df.to_csv(SUM_DIR / "exp15_decoder_stage2.csv", index=False); log_saved(SUM_DIR / "exp15_decoder_stage2.csv"); display(stage2_df.head(10))

    stage3_rows = []
    for bi, base in enumerate(stage2_df.head(3 if not FAST_MODE else 2).to_dict("records")):
        for j, pen in EXP12B_STAGE3_GRID_TRUE.iterrows():
            cfg = {k: base[k] for k in ["decoder", "source_policy", "blend_ordinal", "max_true", "max_pred", "scale", "offset"] if k in base}
            cfg.update({"lambda_gd": float(pen["lambda_gd"]), "lambda_total": float(pen["lambda_total"]), "lambda_outcome": float(pen["lambda_outcome"])})
            stage3_rows.append(exp12b_eval_native_config(cfg, f"stage3_{bi}_{j}"))
    stage3_df = pd.DataFrame(stage3_rows).sort_values("awmae").reset_index(drop=True)
    stage3_df.to_csv(SUM_DIR / "exp15_decoder_stage3.csv", index=False); log_saved(SUM_DIR / "exp15_decoder_stage3.csv"); display(stage3_df.head(10))

    best = stage3_df.iloc[0].to_dict()
    EXP12B_NATIVE_SELECTED_CONFIG = {k: best[k] for k in ["decoder", "source_policy", "blend_ordinal", "max_true", "max_pred", "scale", "offset", "lambda_gd", "lambda_total", "lambda_outcome"] if k in best}
    EXP12B_NATIVE_SELECTED_CONFIG["validation_awmae"] = float(best["awmae"])
    EXP12B_NATIVE_SELECTED_CONFIG["selected_from"] = "stage3_true_native"
else:
    EXP12B_STAGE1_GRID_TRUE.to_csv(SUM_DIR / "exp15_decoder_stage1.csv", index=False)
    EXP12B_STAGE2_GRID_TRUE.to_csv(SUM_DIR / "exp15_decoder_stage2.csv", index=False)
    EXP12B_STAGE3_GRID_TRUE.to_csv(SUM_DIR / "exp15_decoder_stage3.csv", index=False)

with open(SUM_DIR / "exp15_decoder_selected_config.json", "w", encoding="utf-8") as f:
    json.dump(json_safe(EXP12B_NATIVE_SELECTED_CONFIG), f, indent=2)
log_saved(SUM_DIR / "exp15_decoder_selected_config.json")
display(pd.DataFrame([EXP12B_NATIVE_SELECTED_CONFIG]))

EXP12B_NATIVE_SUBMISSIONS = {}
EXP12B_NATIVE_REPORTS = {}
if bool(PAIR_NATIVE_DECODE) and bool(EXP15_ENABLED) and "exp15_test_pmf_raw" in globals():
    native_exp15_sub, native_exp15_match, native_exp15_report = exp12b_decode_true_native_submission(
        test_raw.reset_index(drop=True), exp15_test_pmf_raw, test_pred_team_raw, test_pred_opp_raw, EXP12B_NATIVE_SELECTED_CONFIG, f"{EXP12B_VARIANT}_exp15_true_native"
    )
    EXP12B_NATIVE_SUBMISSIONS["exp15"] = native_exp15_sub
    EXP12B_NATIVE_REPORTS["exp15"] = native_exp15_report
    native_exp15_match.to_csv(PRED_DIR / f"test_pred_{EXP12B_VARIANT}_exp15_true_native_match.csv", index=False)
    log_saved(PRED_DIR / f"test_pred_{EXP12B_VARIANT}_exp15_true_native_match.csv")

    # EXP17 native currently starts from true-native EXP15; W-calibration can still be compared via row-level EXP17.
    # This keeps pair-native variants honest: no export-level averaging repair is used.
    if bool(EXP17_ENABLED):
        EXP12B_NATIVE_SUBMISSIONS["exp17"] = native_exp15_sub.copy()
        EXP12B_NATIVE_REPORTS["exp17"] = pd.DataFrame([{
            "variant": f"{EXP12B_VARIANT}_exp17_true_native",
            "native_decode_mode": "true_match_level_pmf_mbr_exp15_base_m_frozen",
            "pair_repair_count": 0,
            "native_pair_ok": True,
        }])

# Override old helper used by Section 18. It now returns true-native PMF decode output when available,
# and only falls back to the old row-level prediction if EXP15 artifacts do not exist.
def exp12b_native_pair_consistent_submission(row_prediction: pd.DataFrame, row_df: pd.DataFrame, label: str):
    source = "exp17" if "exp17" in str(label).lower() and "exp17" in EXP12B_NATIVE_SUBMISSIONS else "exp15"
    if source in EXP12B_NATIVE_SUBMISSIONS:
        return EXP12B_NATIVE_SUBMISSIONS[source].copy(), EXP12B_NATIVE_REPORTS[source].copy()
    log_info("True-native artifacts not available; using existing row-level prediction without averaging repair.")
    return normalize_submission_columns(row_prediction), pd.DataFrame([{"variant": label, "native_decode_mode": "fallback_row_level", "pair_repair_count": np.nan, "native_pair_ok": False}])

# %% [markdown]
# # 18. EXP12B Submission Export, Pair-Native Check, dan Final Decision
#
# Bagian ini menyimpan submission untuk variant aktif, melakukan native pair export jika flag aktif,
# lalu menjalankan sanity check lengkap. Semua output disimpan sebagai CSV agar bisa diaudit lokal.
# GT tidak digunakan dalam pemilihan config di sini.

# %%
# ============================================================
# 18. EXP12B submission export, sanity check, and final decision
# ============================================================
...
def normalize_submission_columns(sub_df: pd.DataFrame) -> pd.DataFrame:
    out = sub_df.copy()
    if ID_COL not in out.columns and "id" in out.columns:
        out = out.rename(columns={"id": ID_COL})
    if ID_COL not in out.columns:
        raise KeyError(f"Submission missing id column {ID_COL}/id. Columns={list(out.columns)}")
...
def check_submission_pair_consistency(sub_df: pd.DataFrame, row_df: pd.DataFrame) -> tuple[bool, int]:
    sub = normalize_submission_columns(sub_df)
    test_ids = row_df[[ID_COL, MATCH_COL]].copy()
    test_ids["_id_key"] = test_ids[ID_COL].astype(str)
    sub["_id_key"] = sub[ID_COL].astype(str)
    merged = sub.merge(test_ids[["_id_key", MATCH_COL]], on="_id_key", how="left", validate="one_to_one")
...
def align_submission_to_sample(sub_df: pd.DataFrame) -> pd.DataFrame:
    sub = normalize_submission_columns(sub_df)
    if sample_id_col != ID_COL:
        sample_aligned = sample_submission[[sample_id_col]].rename(columns={sample_id_col: ID_COL})
    else:
        sample_aligned = sample_submission[[sample_id_col]].copy()
    temp = sample_aligned.copy()
    temp["_id_key"] = temp[ID_COL].astype(str)
    sub["_id_key"] = sub[ID_COL].astype(str)
    aligned = temp.merge(sub[["_id_key", "team_goals", "opp_goals"]], on="_id_key", how="left", validate="one_to_one").drop(columns=["_id_key"])
    aligned = aligned[[ID_COL, "team_goals", "opp_goals"]].copy()
    aligned["team_goals"] = pd.to_numeric(aligned["team_goals"], errors="raise").astype(int).clip(lower=0)
    aligned["opp_goals"] = pd.to_numeric(aligned["opp_goals"], errors="raise").astype(int).clip(lower=0)
    return aligned

def validate_and_save_exp12b_submission(sub_df: pd.DataFrame, label: str, native_pair: bool = False):
    source_sub = normalize_submission_columns(sub_df)
    pair_ok_before, bad_before = check_submission_pair_consistency(source_sub, test_raw)
    native_report = None
    if native_pair:
        source_sub, native_report = exp12b_native_pair_consistent_submission(source_sub, test_raw, label)
        native_report.to_csv(SUM_DIR / f"native_pair_report_{label}.csv", index=False)
        log_saved(SUM_DIR / f"native_pair_report_{label}.csv")
    aligned = align_submission_to_sample(source_sub)
    pair_ok_after, bad_after = check_submission_pair_consistency(aligned, test_raw)
    sample_ids = sample_submission[sample_id_col].astype(str).tolist()
    checks = [
        {"variant": label, "check": "shape_matches_sample", "passed": tuple(aligned.shape) == (len(sample_submission), 3)},
        {"variant": label, "check": "id_order_matches_sample", "passed": aligned[ID_COL].astype(str).tolist() == sample_ids},
        {"variant": label, "check": "no_missing", "passed": not aligned[["team_goals", "opp_goals"]].isna().any().any()},
        {"variant": label, "check": "no_duplicate_id", "passed": not aligned[ID_COL].duplicated().any()},
        {"variant": label, "check": "non_negative", "passed": bool((aligned[["team_goals", "opp_goals"]] >= 0).all().all())},
        {"variant": label, "check": "pair_consistency", "passed": pair_ok_after},
        {"variant": label, "check": "native_pair_enabled", "passed": bool(native_pair) if bool(PAIR_NATIVE_DECODE) else True},
        {"variant": label, "check": "native_bad_pairs_after", "passed": int(bad_after) == 0},
        {"variant": label, "check": "max_goal_sanity_le_40", "passed": bool((aligned[["team_goals", "opp_goals"]] <= 40).all().all())},
    ]
    check_df = pd.DataFrame(checks)
    check_df["bad_pairs_before"] = int(bad_before)
    check_df["bad_pairs_after"] = int(bad_after)
    check_df["pair_repair_count"] = 0 if native_pair else int(bad_before)
    path = SUB_DIR / f"submission_exp12b_{label}.csv"
    ...
    return aligned, path, check_df, native_report
...
if "submission" in globals():
    sub_base, path_base, check_base, report_base = validate_and_save_exp12b_submission(submission, f"{EXP12B_VARIANT}_advanced_base", native_pair=False)
    submission_catalog.append({"label": f"{EXP12B_VARIANT}_advanced_base", "path": str(path_base), "source": "advanced_base"})
    submission_check_frames.append(check_base)

if "submission_exp15" in globals():
    sub_exp15, path_exp15, check_exp15, report_exp15 = validate_and_save_exp12b_submission(submission_exp15, f"{EXP12B_VARIANT}_exp15", native_pair=bool(PAIR_NATIVE_DECODE))
    submission_catalog.append({"label": f"{EXP12B_VARIANT}_exp15", "path": str(path_exp15), "source": "exp15"})
    submission_check_frames.append(check_exp15)
    if report_exp15 is not None:
        native_reports.append(report_exp15)

if "submission_exp17" in globals():
    sub_exp17, path_exp17, check_exp17, report_exp17 = validate_and_save_exp12b_submission(submission_exp17, f"{EXP12B_VARIANT}_exp17", native_pair=bool(PAIR_NATIVE_DECODE))
    submission_catalog.append({"label": f"{EXP12B_VARIANT}_exp17", "path": str(path_exp17), "source": "exp17"})
    submission_check_frames.append(check_exp17)
    if report_exp17 is not None:
        native_reports.append(report_exp17)

selected_source, primary_sub, primary_source_key = exp12b_select_primary_submission()
primary_submission, primary_path, primary_check, primary_native_report = validate_and_save_exp12b_submission(primary_sub, EXP12B_VARIANT, native_pair=bool(PAIR_NATIVE_DECODE))
submission_catalog.append({"label": EXP12B_VARIANT, "path": str(primary_path), "source": selected_source})
submission_check_frames.append(primary_check)
if primary_native_report is not None:
    native_reports.append(primary_native_report)

best_safe_path = SUB_DIR / "submission_exp12b_best_safe.csv"
primary_submission.to_csv(best_safe_path, index=False)
log_saved(best_safe_path)

submission_catalog_df = pd.DataFrame(submission_catalog).drop_duplicates(subset=["label", "path"]).reset_index(drop=True)
...
submission_check_df = pd.concat(submission_check_frames, ignore_index=True).drop_duplicates()
...
if native_reports:
    pair_report_df = pd.concat(native_reports, ignore_index=True)
else:
    pair_report_df = pd.DataFrame([{"variant": EXP12B_VARIANT, "native_pair_ok": not bool(PAIR_NATIVE_DECODE), "pair_repair_count": int(primary_check["pair_repair_count"].max())}])
pair_report_df.to_csv(SUM_DIR / "pair_consistency_report.csv", index=False)
log_saved(SUM_DIR / "pair_consistency_report.csv")
display(pair_report_df.head(20))

scoreline_distribution = (
    primary_submission
    .assign(scoreline=lambda d: d["team_goals"].astype(int).astype(str) + "-" + d["opp_goals"].astype(int).astype(str))
    ["scoreline"]
    .value_counts()
    .head(30)
    .reset_index()
)
scoreline_distribution.columns = ["scoreline", "count"]
scoreline_distribution.to_csv(SUM_DIR / "scoreline_distribution.csv", index=False)
log_saved(SUM_DIR / "scoreline_distribution.csv")
display(scoreline_distribution.head(15))

variant_metrics_df = pd.DataFrame([{
    "variant": EXP12B_VARIANT,
    "selected_source": selected_source,
    "pair_native_decode": bool(PAIR_NATIVE_DECODE),
    "decoder_calibration": bool(DECODER_CALIBRATION),
    "exp17_w_calibration": bool(EXP17_W_CALIBRATION),
    "use_pseudo_labelling": bool(USE_PSEUDO_LABELLING),
    "use_extreme_overlay": bool(USE_EXTREME_OVERLAY),
    "mean_pred_total": float((primary_submission["team_goals"] + primary_submission["opp_goals"]).mean()),
    "max_pred_goal": int(primary_submission[["team_goals", "opp_goals"]].max().max()),
    "top1_scoreline_share": float(scoreline_distribution.iloc[0]["count"] / len(primary_submission)) if len(scoreline_distribution) else np.nan,
    "top3_scoreline_share": float(scoreline_distribution.head(3)["count"].sum() / len(primary_submission)) if len(scoreline_distribution) else np.nan,
    "pair_consistency_pass": bool(primary_check.loc[primary_check["check"].eq("pair_consistency"), "passed"].iloc[0]),
    "pair_repair_count": int(primary_check["pair_repair_count"].max()),
}])
...
try:
    _tmp = primary_submission.copy()
    _tmp["_id_key"] = _tmp[ID_COL].astype(str)
    _gender_map = test_raw[[ID_COL, "gender"]].copy()
    _gender_map["_id_key"] = _gender_map[ID_COL].astype(str)
    _tmp = _tmp.merge(_gender_map[["_id_key", "gender"]], on="_id_key", how="left", validate="one_to_one")
    gender_metrics_df = _tmp.groupby("gender", dropna=False).agg(
        n_rows=(ID_COL, "count"),
        mean_team_goals=("team_goals", "mean"),
        mean_opp_goals=("opp_goals", "mean"),
        max_goal=("team_goals", "max"),
    ).reset_index()
    ...
    log_info(f"gender metrics skipped: {e!r}")

final_decision = {
    "experiment": "EXP12B",
    "variant": EXP12B_VARIANT,
    "selected_by_validation": selected_source,
    "selected_by_safety": selected_source,
    "main_candidate_submission": str(best_safe_path),
    "source_pipeline": "EXP12B friend rebase",
    "pair_native_decode": bool(PAIR_NATIVE_DECODE),
    "no_pseudo_mainline": not bool(USE_PSEUDO_LABELLING),
    "note": (
        "EXP12B refines EXP12B with no-pseudo, pair-native export, and staged decoder calibration diagnostics. "
        "GT audit is disabled by default and must not be used for training/tuning/selection."
    ),
}
...
log_info(f"variant: {EXP12B_VARIANT}")
log_info(f"selected_source: {selected_source}")
log_info(f"n_test_rows: {len(primary_submission):,}")
log_info(f"mean_pred_total: {float((primary_submission['team_goals'] + primary_submission['opp_goals']).mean()):.4f}")
log_info(f"max_pred_goal: {int(primary_submission[['team_goals', 'opp_goals']].max().max())}")
# %% [markdown]
# # 19. Runtime Summary dan Catatan Akhir
#
# Bagian ini menutup run EXP12B. Script ini adalah switchboard: untuk menjalankan variant lain,
# ubah `EXP12B_VARIANT` atau environment variable dengan nama yang sama, lalu run ulang.
# Prioritas audit lokal biasanya: `a8_exp15_ordinal`, `a9_exp17_w_specialist`, lalu `a10_full_reproduction`.

# %%
print("EXP12B selesai ✓", flush=True)
print("Variant:", EXP12B_VARIANT, flush=True)
print("Output folder:", OUT_DIR.resolve(), flush=True)
print("Primary submission:", primary_path.resolve(), flush=True)
