# %% [markdown]
# # 00. EXP12G — Ringkasan Eksperimen
#
# Penjelasan bagian:
# EXP12G adalah post-processing refinement di atas EXP12F. Fokusnya memperhalus micro outcome booster F3 melalui cap sweep, support threshold, reliability weighting, transition gating, tournament gating, gender gating, dan tiny exact combo.
#
# Output yang perlu dilihat:
# Cek folder output `PROJECT_ROOT/outputs/exp12g_outcome_booster_refinement/<variant>/`. Kandidat penting akan tersimpan di `submissions/`, metrik sanity di `summaries/`, dan local GT audit ada di cell paling akhir.
...
EXP12G_VARIANT = os.environ.get("EXP12G_VARIANT", "g1_cap_sweep")
OUT_DIR = OUTPUT_ROOT / "exp12g_outcome_booster_refinement" / EXP12G_VARIANT
...
RUN_LOCAL_GT_AUDIT_DEFAULT = os.environ.get("EXP12G_RUN_LOCAL_GT_AUDIT", "1").strip().lower() in {"1", "true", "yes", "y"}
STRICT_FINAL = True
MAX_GOAL_SANITY = 40
...
log_info(f"EXP12G_VARIANT = {EXP12G_VARIANT}")
...
def save_submission(submission_df: pd.DataFrame, label: str, strict: bool = False) -> tuple[Path, pd.DataFrame]:
    ...
    path = SUB_DIR / f"submission_exp12g_{label}.csv"
...
# %% [markdown]
# # 04. Load EXP12F Champion dan Candidate Pool
#
# Penjelasan bagian:
# Bagian ini mencari EXP12F champion `f3_draw_to_win_cap020` sebagai G0 dan mencari EXP12E best sebagai pre-booster base untuk cap sweep. Candidate lain dimuat sebagai comparison artifact, bukan fitur training.
#
# Output yang perlu dilihat:
# `g0_reproduction_summary.csv` harus menunjukkan G0 berasal dari `f3_draw_to_win_cap020`. Jika G0 tidak ketemu dan fallback terjadi, hasil G1-G7 harus dibaca hati-hati.
...
champ_label, champion_submission, champ_path = pick_candidate([
    ...
    "exp12f_f3_draw_to_win_cap0200",
    "f3_draw_to_win_cap020",
    "f3_draw_to_win_cap0200",
...
if champ_label is None:
    log_info("EXP12F champion f3 cap020 tidak ditemukan. Fallback ke EXP12E best/pre-booster base.")
...
g0_summary = pd.DataFrame([
    {"role": "g0_exp12f_champion", "label": champ_label, "path": str(champ_path)},
...
champion_match = submission_to_match_df(champion_base_submission, label="g0_champion")
...
save_candidate(champion_base_submission, "g0_exp12f_f3_cap020_reproduction", strict=False)
...
def build_candidate_match_pool(max_candidates: int = 32) -> dict[str, pd.DataFrame]:
    pool = {
        "g0_exp12f_champion": submission_to_match_df(champion_base_submission, "g0_exp12f_champion"),
        "pre_booster_base": submission_to_match_df(base_submission, "pre_booster_base"),
    }
    priority_terms = [
        "exp12f", "f3", "cap020", "exp12e", "consensus", "min_total", "preserve_gd",
...
#
# Penjelasan bagian:
# Bagian ini membuat fungsi untuk mencari alternative scoreline dari candidate pool. EXP12G menambahkan support threshold, reliability weight, transition filter, tournament filter, gender filter, dan tiny exact combo.
#
# Output yang perlu dilihat:
# Bagian ini tidak punya output besar. Grid G1-G7 akan menunjukkan hasilnya.

# %%
def candidate_reliability_weight(label: str) -> float:
    l = str(label).lower()
    if "g0_exp12f_champion" in l or "f3" in l or "cap020" in l:
        return 3.0
...
def apply_outcome_booster(
    ...
    variant: str,
    cap_rate: float = 0.02,
    support_threshold: int | callable = 2,
    weighted_support_threshold: float | callable | None = None,
    extra_filter=None,
    trusted_only: bool = False,
    exclude_exp17: bool = False,
    cap_rate_by_gender: dict[str, float] | None = None,
...
# %% [markdown]
# # 07. G1 — Fine Cap Sweep Around 2%
#
# Penjelasan bagian:
# G1 mengulang logic F3 draw-to-win, tetapi cap disweep halus di sekitar 2%. Ini mencari sweet spot antara cap 1.25% sampai 2.75%.
#
# Output yang perlu dilihat:
# `cap_sweep_grid.csv` menunjukkan changed rate, high/low weight changes, dan pair consistency. Candidate cap0200 harus merepresentasikan EXP12F F3 cap020 style.

# %%
G1_CAPS = [0.0125, 0.0150, 0.0175, 0.0200, 0.0225, 0.0250, 0.0275]
g1_records, g1_submissions = [], {}
for cap in G1_CAPS:
    label = f"g1_draw_to_win_cap{int(round(cap * 10000)):04d}"
    ...
    g1_records.append(rec)
    g1_submissions[label] = sub
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

cap_sweep_grid = pd.DataFrame(g1_records).sort_values("cap_rate")
cap_sweep_grid.to_csv(SUM_DIR / "cap_sweep_grid.csv", index=False)
log_saved(SUM_DIR / "cap_sweep_grid.csv")
display(cap_sweep_grid)

# %% [markdown]
# # 08. G2 — Support Threshold Tuning
#
# Penjelasan bagian:
# G2 menguji support 2/3/4. Support lebih tinggi membuat booster lebih bersih, terutama kalau cap dinaikkan sedikit.
#
# Output yang perlu dilihat:
# `support_threshold_grid.csv`. Support3 cap0200/cap0225 biasanya kandidat kunci.

# %%
G2_CONFIGS = [
    (2, 0.0175), (2, 0.0200), (2, 0.0225),
    (3, 0.0175), (3, 0.0200), (3, 0.0225),
    (4, 0.0200),
]
g2_records = []
for support, cap in G2_CONFIGS:
    label = f"g2_support{support}_cap{int(round(cap * 10000)):04d}"
    ...
    g2_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

support_threshold_grid = pd.DataFrame(g2_records)
support_threshold_grid.to_csv(SUM_DIR / "support_threshold_grid.csv", index=False)
log_saved(SUM_DIR / "support_threshold_grid.csv")
display(support_threshold_grid)

# %% [markdown]
# # 09. G3 — Candidate Reliability Weighting
#
# Penjelasan bagian:
# G3 memberi bobot lebih besar pada candidate yang terbukti kuat, seperti EXP12F F3 dan EXP12E best family. Weighted support dapat mengurangi noise dari kandidat lama/raw.
#
# Output yang perlu dilihat:
# `reliability_weight_grid.csv` dan `candidate_reliability_map.csv`.

# %%
G3_CONFIGS = [
    ("g3_weighted_support_3p0_cap0200", 3.0, 0.0200, False, False),
    ("g3_weighted_support_3p5_cap0200", 3.5, 0.0200, False, False),
    ("g3_weighted_support_4p0_cap0225", 4.0, 0.0225, False, False),
    ("g3_trusted_only_cap0200", None, 0.0200, True, False),
    ("g3_exclude_exp17_cap0200", None, 0.0200, False, True),
]
g3_records = []
for label, wthr, cap, trusted_only, exclude_exp17 in G3_CONFIGS:
    sub, changes = apply_outcome_booster(base_submission, label, cap_rate=cap, support_threshold=2, weighted_support_threshold=wthr, trusted_only=trusted_only, exclude_exp17=exclude_exp17)
    path, check = save_candidate(sub, label, strict=False)
    rec = compare_to_base(sub, base_submission, label)
    rec.update({"weighted_support_threshold": wthr, "cap_rate": cap, "trusted_only": trusted_only, "exclude_exp17": exclude_exp17, "path": str(path), "pair_consistency": bool(check.loc[check["check"] == "pair_consistency", "passed"].iloc[0])})
    g3_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

reliability_weight_grid = pd.DataFrame(g3_records)
reliability_weight_grid.to_csv(SUM_DIR / "reliability_weight_grid.csv", index=False)
log_saved(SUM_DIR / "reliability_weight_grid.csv")
display(reliability_weight_grid)

# %% [markdown]
# # 10. G4 — Transition-Specific Outcome Booster
#
# Penjelasan bagian:
# G4 memisahkan draw transition: 0-0, 1-1, 2-2, low-total, exclude 0-0, dan exclude 2-2. Ini membantu mencari transition mana yang paling bersih.
#
# Output yang perlu dilihat:
# `transition_specific_grid.csv` dan `transition_specific_analysis.csv`.

# %%
def score_filter(a=None, b=None, not_score=None, max_total=None):
    def _f(r):
...
G4_CONFIGS = [
    ("g4_only_0_0_draws_cap0100", 0.0100, score_filter(0, 0)),
    ("g4_only_1_1_draws_cap0200", 0.0200, score_filter(1, 1)),
    ("g4_only_2_2_draws_cap0100", 0.0100, score_filter(2, 2)),
    ("g4_no_2_2_transition_cap0200", 0.0200, score_filter(not_score=(2, 2))),
    ("g4_low_total_draw_only_cap0200", 0.0200, score_filter(max_total=2)),
    ("g4_exclude_0_0_cap0200", 0.0200, score_filter(not_score=(0, 0))),
]
g4_records = []
for label, cap, filt in G4_CONFIGS:
    sub, changes = apply_outcome_booster(base_submission, label, cap_rate=cap, support_threshold=2, extra_filter=filt)
    path, check = save_candidate(sub, label, strict=False)
    rec = compare_to_base(sub, base_submission, label)
    rec.update({"cap_rate": cap, "path": str(path), "pair_consistency": bool(check.loc[check["check"] == "pair_consistency", "passed"].iloc[0])})
    g4_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

transition_specific_grid = pd.DataFrame(g4_records)
transition_specific_grid.to_csv(SUM_DIR / "transition_specific_grid.csv", index=False)
log_saved(SUM_DIR / "transition_specific_grid.csv")
display(transition_specific_grid)

transition_specific_analysis = transition_table({k: ALL_CHANGE_DETAILS[k] for k in [r["variant"] for r in g4_records]})
transition_specific_analysis.to_csv(SUM_DIR / "transition_specific_analysis.csv", index=False)
log_saved(SUM_DIR / "transition_specific_analysis.csv")

# %% [markdown]
# # 11. G5 — Tournament-Aware Outcome Booster
#
# Penjelasan bagian:
# G5 membuat high-weight tournament lebih ketat dan low-weight/friendly bisa lebih fleksibel.
#
# Output yang perlu dilihat:
# `tournament_aware_grid.csv` dan `tournament_change_breakdown.csv`.
...
G5_CONFIGS = [
    ("g5_no_high_weight_changes_cap0200", 0.0200, lambda r: float(r.get("tournament_weight", 1.2)) < 1.8, 2),
    ("g5_high_weight_support3_else_support2_cap0200", 0.0200, None, lambda r: 3 if float(r.get("tournament_weight", 1.2)) >= 1.8 else 2),
    ("g5_high_weight_cap005_global_cap0200", 0.0200, None, lambda r: 3 if float(r.get("tournament_weight", 1.2)) >= 1.8 else 2),
    ("g5_low_weight_more_aggressive_cap0250", 0.0250, None, 2),
    ("g5_world_asian_strict_support3_cap0200", 0.0200, None, lambda r: 3 if is_world_asian_tournament(r.get("tournament", "")) else 2),
    ("g5_friendly_only_extra_cap0100", 0.0100, lambda r: float(r.get("tournament_weight", 1.2)) <= 0.96, 2),
]
g5_records = []
for label, cap, filt, support_thr in G5_CONFIGS:
    sub, changes = apply_outcome_booster(base_submission, label, cap_rate=cap, support_threshold=support_thr, extra_filter=filt)
    path, check = save_candidate(sub, label, strict=False)
    rec = compare_to_base(sub, base_submission, label)
    rec.update({"cap_rate": cap, "path": str(path), "pair_consistency": bool(check.loc[check["check"] == "pair_consistency", "passed"].iloc[0])})
    g5_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

tournament_aware_grid = pd.DataFrame(g5_records)
tournament_aware_grid.to_csv(SUM_DIR / "tournament_aware_grid.csv", index=False)
log_saved(SUM_DIR / "tournament_aware_grid.csv")
display(tournament_aware_grid)

tournament_change_breakdown_rows = []
for row in g5_records:
    label = row["variant"]
    ch = ALL_CHANGE_DETAILS.get(label, pd.DataFrame())
    if ch is None or len(ch) == 0:
        tournament_change_breakdown_rows.append({"variant": label, "bucket": "none", "n_changed": 0})
        continue
    tmp = ch.copy()
    tmp["bucket"] = np.where(tmp["tournament_weight"] >= 1.8, "high_weight", np.where(tmp["tournament_weight"] <= 0.96, "low_weight", "medium_weight"))
    for b, g in tmp.groupby("bucket"):
        tournament_change_breakdown_rows.append({"variant": label, "bucket": b, "n_changed": len(g)})
tournament_change_breakdown = pd.DataFrame(tournament_change_breakdown_rows)
tournament_change_breakdown.to_csv(SUM_DIR / "tournament_change_breakdown.csv", index=False)
log_saved(SUM_DIR / "tournament_change_breakdown.csv")

# %% [markdown]
# # 12. G6 — Gender-Aware Outcome Booster
#
# Penjelasan bagian:
# G6 menguji apakah booster sebaiknya M-only, W-only, atau beda cap/support antara M dan W.
#
# Output yang perlu dilihat:
# `gender_aware_grid.csv` dan `gender_change_breakdown.csv`.

# %%
G6_CONFIGS = [
    ("g6_m_only_cap0200", None, {"M": 0.0200, "W": 0.0}, lambda r: str(r.get("gender", "")).upper() == "M", 2),
    ("g6_w_only_cap0100", None, {"M": 0.0, "W": 0.0100}, lambda r: str(r.get("gender", "")).upper() == "W", 2),
    ("g6_m_cap0200_w_cap0050", None, {"M": 0.0200, "W": 0.0050}, None, 2),
    ("g6_m_cap0225_w_cap0000", None, {"M": 0.0225, "W": 0.0}, lambda r: str(r.get("gender", "")).upper() == "M", 2),
    ("g6_m_support2_w_support3_cap0200", 0.0200, None, None, lambda r: 3 if str(r.get("gender", "")).upper() == "W" else 2),
]
g6_records = []
for label, cap, cap_by_gender, filt, support_thr in G6_CONFIGS:
    sub, changes = apply_outcome_booster(base_submission, label, cap_rate=cap or 0.0200, cap_rate_by_gender=cap_by_gender, support_threshold=support_thr, extra_filter=filt)
    path, check = save_candidate(sub, label, strict=False)
    rec = compare_to_base(sub, base_submission, label)
    rec.update({"cap_rate": cap, "cap_rate_by_gender": json.dumps(cap_by_gender) if cap_by_gender else "", "path": str(path), "pair_consistency": bool(check.loc[check["check"] == "pair_consistency", "passed"].iloc[0])})
    g6_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

gender_aware_grid = pd.DataFrame(g6_records)
gender_aware_grid.to_csv(SUM_DIR / "gender_aware_grid.csv", index=False)
log_saved(SUM_DIR / "gender_aware_grid.csv")
display(gender_aware_grid)

gender_change_breakdown_rows = []
for row in g6_records:
    label = row["variant"]
    ch = ALL_CHANGE_DETAILS.get(label, pd.DataFrame())
    if ch is None or len(ch) == 0:
        gender_change_breakdown_rows.append({"variant": label, "gender": "none", "n_changed": 0})
        continue
    for gender, g in ch.groupby("gender"):
        gender_change_breakdown_rows.append({"variant": label, "gender": gender, "n_changed": len(g)})
gender_change_breakdown = pd.DataFrame(gender_change_breakdown_rows)
gender_change_breakdown.to_csv(SUM_DIR / "gender_change_breakdown.csv", index=False)
log_saved(SUM_DIR / "gender_change_breakdown.csv")

# %% [markdown]
# # 13. G7 — F3 + Tiny Exact Combo
#
# Penjelasan bagian:
# G7 mengambil outcome booster candidate lalu menambahkan exact booster sangat kecil. Exact booster tidak boleh menyentuh match yang sudah diubah outcome booster.
#
# Output yang perlu dilihat:
# `tiny_exact_combo_grid.csv` dan `tiny_exact_combo_transition_analysis.csv`. Additional change harus sangat kecil dan pair-consistent.

# %%
G7_BASES = [lab for lab in ["g1_draw_to_win_cap0200", "g1_draw_to_win_cap0225", "g2_support3_cap0200"] if lab in ALL_CANDIDATE_SUBMISSIONS]
G7_CONFIGS = []
for base_label in G7_BASES:
    for exact_cap in ([0.0025, 0.0050, 0.0075] if base_label == "g1_draw_to_win_cap0200" else [0.0025, 0.0050]):
        label = f"g7_{base_label}_plus_exact_cap{int(round(exact_cap * 10000)):04d}"
        G7_CONFIGS.append((label, base_label, exact_cap))

g7_records = []
for label, base_label, exact_cap in G7_CONFIGS:
    ...
    path, check = save_candidate(sub, label, strict=False)
    rec = compare_to_base(sub, base_submission, label)
    rec.update({"base_outcome_variant": base_label, "exact_cap": exact_cap, "path": str(path), "pair_consistency": bool(check.loc[check["check"] == "pair_consistency", "passed"].iloc[0])})
    g7_records.append(rec)
    ALL_CANDIDATE_SUBMISSIONS[label] = sub
    ALL_CHANGE_DETAILS[label] = changes

tiny_exact_combo_grid = pd.DataFrame(g7_records)
tiny_exact_combo_grid.to_csv(SUM_DIR / "tiny_exact_combo_grid.csv", index=False)
log_saved(SUM_DIR / "tiny_exact_combo_grid.csv")
if len(tiny_exact_combo_grid):
    display(tiny_exact_combo_grid)

tiny_exact_combo_transition_analysis = transition_table({r["variant"]: ALL_CHANGE_DETAILS[r["variant"]] for r in g7_records})
tiny_exact_combo_transition_analysis.to_csv(SUM_DIR / "tiny_exact_combo_transition_analysis.csv", index=False)
log_saved(SUM_DIR / "tiny_exact_combo_transition_analysis.csv")

# %% [markdown]
# # 14. Combined Sanity Metrics dan Scoreline Distribution
#
# Penjelasan bagian:
# Bagian ini menggabungkan semua candidate G0-G7 ke tabel sanity GT-free. Ini membantu mendeteksi candidate yang terlalu agresif sebelum audit lokal.
#
# Output yang perlu dilihat:
...
# %%
preferred_order = [
    "g1_draw_to_win_cap0200",
    "g2_support3_cap0200",
    "g1_draw_to_win_cap0175",
    "g3_weighted_support_3p5_cap0200",
    "g5_high_weight_support3_else_support2_cap0200",
    "g6_m_cap0200_w_cap0050",
    "g0_exp12f_f3_cap020_reproduction",
]
selected_label = None
...
if selected_label is None:
    selected_label = "g0_exp12f_f3_cap020_reproduction"
...
final_decision = {
    ...
    "recommended_for_local_audit": "all generated submission_exp12g_*.csv; champion is decided manually from Section 99 local audit",
    "main_submission_path": str(best_path),
    "g0_base_label": str(champ_label),
