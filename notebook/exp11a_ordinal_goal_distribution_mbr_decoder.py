# %% [markdown]
# # 00. EXP11A — Ringkasan Eksperimen
#
# Bagian ini menjelaskan tujuan EXP11A. Eksperimen ini melanjutkan EXP09D sebagai current best
# dan menambahkan ordinal goal distribution + MBR decoder tanpa memakai GT test, external data,
# atau submission lama sebagai fitur. Goal/tail/outcome gate tetap mengikuti EXP09D; yang diuji
# adalah representasi distribusi gol dan cara decode scoreline berbasis expected AW-MAE.
...
# XGBoost/TabPFN/tqdm, dan output disimpan ke folder EXP11A.
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'exp11a_ordinal_goal_distribution_mbr_decoder'
...
log_info('EXP11A source of truth: EXP05A-LITE-FIX-V2 with EXP09D as current best mainline. EXP09A repair is used only for outcome repair functions.')
...
# probability outcome repair dipakai berdasarkan tournament_structure. EXP11A memakai gate terpilih
...
# %% [markdown]
# # 13. EXP09D Gate Selection sebagai Baseline EXP11A
#
...
log_section('EXP09D gate selection baseline for EXP11A')
GATE_RESULTS = {}
gate_metric_rows = []
v0_detail = None
...
# %% [markdown]
# # 17. MBR Decoder Variants
#
# Bagian ini membuat Minimum Bayes Risk decoder. Decoder memilih scoreline dengan expected AW-MAE
# terkecil berdasarkan joint PMF scoreline. Variant yang dibandingkan adalah EXP09D original,
# ordinal all-gender, ordinal M-only, dan PMF blend.
...
def mbr_decode_from_pmfs(raw_df, pmf_a, pmf_b, max_decode_goals=9, outcome_weight=0.0, total_anchor_weight=0.0, gd_anchor_weight=0.0):
    candidates = [(a, b) for a in range(max_decode_goals + 1) for b in range(max_decode_goals + 1)]
    ...
    joint_flat = joint_flat / joint_flat.sum(axis=1, keepdims=True)
    ...
    if total_anchor_weight > 0:
        cont_total = (pd.to_numeric(raw_df['pred_goal_a_cont'], errors='coerce').fillna(1.0) + pd.to_numeric(raw_df['pred_goal_b_cont'], errors='coerce').fillna(1.0)).to_numpy(float)
        risks += float(total_anchor_weight) * np.abs((cand_a + cand_b)[None, :] - cont_total[:, None])
    if gd_anchor_weight > 0:
        cont_gd = (pd.to_numeric(raw_df['pred_goal_a_cont'], errors='coerce').fillna(1.0) - pd.to_numeric(raw_df['pred_goal_b_cont'], errors='coerce').fillna(1.0)).to_numpy(float)
        risks += float(gd_anchor_weight) * np.abs((cand_a - cand_b)[None, :] - cont_gd[:, None])
    idx = np.argmin(risks, axis=1)
    return pd.DataFrame({'match_id': raw_df['match_id'].values, 'pred_team_a_goals': cand_a[idx].astype(int), 'pred_team_b_goals': cand_b[idx].astype(int), 'mbr_risk': risks[np.arange(len(raw_df)), idx].astype(float)})

MBR_VARIANTS = [
    {'variant_name': 'V0_exp09d_original', 'decoder_type': 'exp09d_original'},
    {'variant_name': 'V1_ordinal_all', 'decoder_type': 'ordinal_mbr', 'gender_policy': 'all', 'blend_ordinal': 1.00, 'outcome_weight': 0.00, 'total_anchor_weight': 0.00, 'gd_anchor_weight': 0.00},
    {'variant_name': 'V2_ordinal_m_only', 'decoder_type': 'gender_gated', 'gender_policy': 'M_ordinal_W_exp09d', 'blend_ordinal': 1.00, 'outcome_weight': 0.00, 'total_anchor_weight': 0.00, 'gd_anchor_weight': 0.00},
    {'variant_name': 'V3_ordinal_blend075_all', 'decoder_type': 'ordinal_mbr', 'gender_policy': 'all', 'blend_ordinal': 0.75, 'outcome_weight': 0.00, 'total_anchor_weight': 0.00, 'gd_anchor_weight': 0.00},
    {'variant_name': 'V4_ordinal_blend075_m_only', 'decoder_type': 'gender_gated', 'gender_policy': 'M_ordinal_W_exp09d', 'blend_ordinal': 0.75, 'outcome_weight': 0.00, 'total_anchor_weight': 0.00, 'gd_anchor_weight': 0.00},
]
with open(SUM_DIR / 'mbr_variant_config.json', 'w', encoding='utf-8') as f:
    json.dump(MBR_VARIANTS, f, indent=2, ensure_ascii=False)
display(pd.DataFrame(MBR_VARIANTS))
log_saved(SUM_DIR / 'mbr_variant_config.json')

# %% [markdown]
# # 18. Variant Evaluation
#
# Bagian ini mengevaluasi V0 EXP09D dan semua MBR ordinal variants pada validation. Metric utama
# disimpan ke CSV dan ditampilkan sebagai DataFrame agar output rapi dan mudah dibandingkan.

# %%
def add_changed_metrics(summary, detail, baseline_detail):
    compare = detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss', 'tournament_weight']].merge(
        baseline_detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss']],
        on='match_id', how='left', suffixes=('', '_v0'), validate='one_to_one'
    )
    changed = (compare['pred_team_a_goals'] != compare['pred_team_a_goals_v0']) | (compare['pred_team_b_goals'] != compare['pred_team_b_goals_v0'])
    delta = compare['official_loss'] - compare['official_loss_v0']
    summary['n_changed_vs_v0'] = int(changed.sum())
    summary['changed_rate_vs_v0'] = float(changed.mean())
    summary['n_improved_changed'] = int(((delta < 0) & changed).sum())
...
def evaluate_mbr_variant(cfg):
    variant_name = cfg['variant_name']
    if cfg['decoder_type'] == 'exp09d_original':
        pred_match = GATE_RESULTS[selected_gate_name]['pred_match'].copy()
        detail = prediction_detail_table(pred_match, variant_name)
        summary = summarize_prediction_detail(detail, variant_name)
        summary = add_changed_metrics(summary, detail, detail)
        return pred_match, detail, summary

    blend = float(cfg.get('blend_ordinal', 1.0))
    pmf_a = blend_pmfs(valid_pmf_ord_a, valid_pmf_pois_a, blend)
    pmf_b = blend_pmfs(valid_pmf_ord_b, valid_pmf_pois_b, blend)
    pred_ord = mbr_decode_from_pmfs(
        raw_gate_valid,
        pmf_a,
        pmf_b,
    ...
    if cfg.get('gender_policy') == 'M_ordinal_W_exp09d':
        base_pred = GATE_RESULTS[selected_gate_name]['pred_match'][['match_id', 'pred_team_a_goals', 'pred_team_b_goals']].copy()
        pred_combo = raw_gate_valid[['match_id', 'gender']].merge(pred_ord, on='match_id', how='left', validate='one_to_one')
        pred_combo = pred_combo.merge(base_pred, on='match_id', how='left', suffixes=('', '_v0'), validate='one_to_one')
        is_m = pred_combo['gender'].astype(str).eq('M')
        pred_combo['pred_team_a_goals'] = np.where(is_m, pred_combo['pred_team_a_goals'], pred_combo['pred_team_a_goals_v0']).astype(int)
        pred_combo['pred_team_b_goals'] = np.where(is_m, pred_combo['pred_team_b_goals'], pred_combo['pred_team_b_goals_v0']).astype(int)
        pred = pred_combo[['match_id', 'pred_team_a_goals', 'pred_team_b_goals']]
    else:
        pred = pred_ord[['match_id', 'pred_team_a_goals', 'pred_team_b_goals']]
    pred_match = raw_gate_valid.merge(pred, on='match_id', how='left', validate='one_to_one')
    detail = prediction_detail_table(pred_match, variant_name)
    summary = summarize_prediction_detail(detail, variant_name)
    summary['blend_ordinal'] = blend
    summary['gender_policy'] = cfg.get('gender_policy', 'all')
    summary['outcome_weight'] = float(cfg.get('outcome_weight', 0.0))
    summary = add_changed_metrics(summary, detail, exp09d_detail)
    return pred_match, detail, summary

log_section('Evaluate MBR variants')
MBR_RESULTS = {}
mbr_metric_rows = []
for cfg in MBR_VARIANTS:
    pred_match, detail, summary = evaluate_mbr_variant(cfg)
    save_name = cfg['variant_name'].lower()
    pred_path = PRED_DIR / f"valid_pred_match_{save_name}.csv"
    pred_match.to_csv(pred_path, index=False)
    log_saved(pred_path)
    MBR_RESULTS[cfg['variant_name']] = {'config': cfg, 'pred_match': pred_match, 'detail': detail, 'summary': summary}
    mbr_metric_rows.append(summary)

mbr_variant_metrics_df = pd.DataFrame(mbr_metric_rows).sort_values('awmae').reset_index(drop=True)
mbr_variant_metrics_df.to_csv(SUM_DIR / 'mbr_variant_metrics.csv', index=False)
display(mbr_variant_metrics_df)
log_saved(SUM_DIR / 'mbr_variant_metrics.csv')

scoreline_dist = scoreline_distribution(pd.concat([v['detail'] for v in MBR_RESULTS.values()], ignore_index=True))
...
# %% [markdown]
# # 19. Domain Error Analysis
#
# Bagian ini mengecek performa ordinal MBR berdasarkan gender, tournament_structure, dan blowout /
# high-total flags. Fokusnya memastikan ordinal tidak memperbaiki base MAE tetapi merusak outcome,
# goal difference, atau subgroup women secara besar.

# %%
log_section('Domain error analysis for MBR variants')
all_mbr_detail_df = pd.concat([v['detail'] for v in MBR_RESULTS.values()], ignore_index=True)
...
domain_gender_df = compute_domain_error_per_variant(all_mbr_detail_df, 'gender')
domain_tournament_df = compute_domain_error_per_variant(all_mbr_detail_df, 'tournament_structure')
domain_blowout_df = pd.concat([
    compute_domain_error_per_variant(all_mbr_detail_df, 'is_blowout_5plus').assign(domain='is_blowout_5plus'),
    compute_domain_error_per_variant(all_mbr_detail_df, 'is_blowout_7plus').assign(domain='is_blowout_7plus'),
    compute_domain_error_per_variant(all_mbr_detail_df, 'is_high_total_6plus').assign(domain='is_high_total_6plus'),
], ignore_index=True)
...
# # 20. Final Decision
#
# Bagian ini memilih variant terbaik berdasarkan AW-MAE dan safety check. Jika ordinal MBR tidak aman,
# final fallback ke EXP09D. Final decision disimpan sebagai JSON dan ditampilkan sebagai DataFrame.

# %%
log_section('Final decision for EXP11A')
metrics = mbr_variant_metrics_df.copy()
v0 = metrics[metrics['variant_name'].eq('V0_exp09d_original')].iloc[0]
metrics['awmae_gain_vs_v0'] = float(v0['awmae']) - metrics['awmae']

safe_candidates = []
for _, row in metrics[~metrics['variant_name'].eq('V0_exp09d_original')].iterrows():
    safe = (
        (float(row['awmae']) < float(v0['awmae']))
        and (float(row['base_mae']) <= float(v0['base_mae']) + 0.002)
        and (float(row['outcome_rate']) >= float(v0['outcome_rate']) - 0.006)
        and (float(row['gd_rate']) >= float(v0['gd_rate']) - 0.008)
        and (float(row['top3_scoreline_share']) <= float(v0['top3_scoreline_share']) + 0.030)
        and (1.0 <= float(row['mean_pred_total']) <= 4.5)
    )
    if safe and 'all' in str(row.get('gender_policy', '')):
        v0_w = domain_gender_df[(domain_gender_df['variant_name'].eq('V0_exp09d_original')) & (domain_gender_df['gender'].astype(str).eq('W'))]
        row_w = domain_gender_df[(domain_gender_df['variant_name'].eq(row['variant_name'])) & (domain_gender_df['gender'].astype(str).eq('W'))]
        if len(v0_w) and len(row_w) and float(row_w['awmae'].iloc[0]) > float(v0_w['awmae'].iloc[0]) + 0.10:
            safe = False
    if safe:
...
if safe_candidates:
    ...
    selected_mbr_name = str(selected_row['variant_name'])
    if 'M_ordinal_W_exp09d' in str(selected_row.get('gender_policy', '')):
        decision_code = 'B'
        decision_text = 'Ordinal improves safely with M-only gender-gated policy.'
    else:
        decision_code = 'A'
        decision_text = 'Ordinal MBR improves AW-MAE and passes safety checks.'
else:
    if str(metrics.sort_values('awmae').iloc[0]['variant_name']) == 'V0_exp09d_original':
        decision_code = 'C'
        decision_text = 'EXP09D baseline remains best.'
    else:
        decision_code = 'D'
        decision_text = 'Ordinal variant has promising metric but fails safety checks, fallback to EXP09D.'
    selected_mbr_name = 'V0_exp09d_original'
    selected_row = metrics[metrics['variant_name'].eq('V0_exp09d_original')].iloc[0]

selected_mbr_config = copy.deepcopy(MBR_RESULTS[selected_mbr_name]['config'])
selected_mbr_json = {
    'experiment': 'EXP11A',
    'base_pipeline': 'EXP05A-LITE-FIX-V2 / EXP09D',
    'selected_gate': selected_gate_name,
    'selected_mbr_variant': selected_mbr_name,
    'selected_mbr_config': selected_mbr_config,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'v0_awmae': float(v0['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_v0': float(float(v0['awmae']) - float(selected_row['awmae'])),
}
with open(SUM_DIR / 'selected_mbr_config.json', 'w', encoding='utf-8') as f:
    json.dump(selected_mbr_json, f, indent=2, ensure_ascii=False)

final_decision = {
    'experiment': 'EXP11A',
    'main_change': 'Ordinal goal distribution and MBR decoder over EXP09D tournament-gated outcome repair',
    'selected_gate': selected_gate_name,
    'selected_mbr_variant': selected_mbr_name,
    'decision_code': decision_code,
    'decision_text': decision_text,
    ...
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_v0': float(float(v0['awmae']) - float(selected_row['awmae'])),
    'caveat': 'Goal/outcome/tail mainline stays EXP09D. Ordinal distribution is only used by selected MBR decoder variant.',
}
...
selected_mbr_config_df = pd.DataFrame([{'key': k, 'value': str(v)} for k, v in selected_mbr_json.items()])
...
display(selected_mbr_config_df)
display(final_decision_df)
log_saved(SUM_DIR / 'selected_mbr_config.json')
log_saved(SUM_DIR / 'final_decision.json')
log_saved(SUM_DIR / 'final_decision_table.csv')

# %% [markdown]
# # 21. Final Model Training dan Test Inference
#
# Bagian ini melatih ulang EXP09D base/repair pada full train, membuat PMF ordinal test jika diperlukan,
# lalu decode sesuai selected variant. Jika V0 terpilih, output secara konsep kembali ke EXP09D.
...
if selected_mbr_name == 'V0_exp09d_original':
    test_pred = test_pred_exp09d.copy()
else:
    ...
    blend = float(selected_mbr_config.get('blend_ordinal', 1.0))
    test_pmf_a = blend_pmfs(test_pmf_ord_a, test_pmf_pois_a, blend)
    test_pmf_b = blend_pmfs(test_pmf_ord_b, test_pmf_pois_b, blend)
    test_pred_ord = mbr_decode_from_pmfs(
        test_raw_gate,
        test_pmf_a,
        test_pmf_b,
        max_decode_goals=MAX_DECODE_GOALS,
        outcome_weight=float(selected_mbr_config.get('outcome_weight', 0.0)),
        total_anchor_weight=float(selected_mbr_config.get('total_anchor_weight', 0.0)),
        gd_anchor_weight=float(selected_mbr_config.get('gd_anchor_weight', 0.0)),
    )
    if selected_mbr_config.get('gender_policy') == 'M_ordinal_W_exp09d':
        combo = test_raw_gate[['match_id', 'gender']].merge(test_pred_ord, on='match_id', how='left', validate='one_to_one')
        combo = combo.merge(test_pred_exp09d, on='match_id', how='left', suffixes=('', '_v0'), validate='one_to_one')
        is_m = combo['gender'].astype(str).eq('M')
        combo['pred_team_a_goals'] = np.where(is_m, combo['pred_team_a_goals'], combo['pred_team_a_goals_v0']).astype(int)
        combo['pred_team_b_goals'] = np.where(is_m, combo['pred_team_b_goals'], combo['pred_team_b_goals_v0']).astype(int)
        test_pred = combo[['match_id', 'pred_team_a_goals', 'pred_team_b_goals']]
    else:
        test_pred = test_pred_ord[['match_id', 'pred_team_a_goals', 'pred_team_b_goals']]
...
test_pred_match_best['selected_mbr_variant'] = selected_mbr_name
test_pred_match_best.to_csv(PRED_DIR / 'test_pred_exp11a_best_safe.csv', index=False)
log_saved(PRED_DIR / 'test_pred_exp11a_best_safe.csv')

print('[TEST INFERENCE SUMMARY]', flush=True)
log_info(f'selected_gate: {selected_gate_name}')
log_info(f'selected_mbr_variant: {selected_mbr_name}')
...
# %% [markdown]
# # 22. Submission Mapping dan Pair Consistency
#
...
sub_path = SUB_DIR / 'submission_exp11a_best_safe.csv'
...
# %% [markdown]
# # 23. Runtime Summary dan Catatan Akhir
#
# Bagian ini menyimpan runtime summary dan menampilkan ringkasan akhir. Output akhir yang perlu dicek
# adalah selected variant, decision code, validation metric, dan path submission final.
...
summary_df = pd.DataFrame([
    {'item': 'experiment', 'value': 'EXP11A'},
    {'item': 'selected_gate', 'value': selected_gate_name},
    {'item': 'selected_mbr_variant', 'value': selected_mbr_name},
...
log_result('EXP11A completed. Check validation safety, domain tables, and pair consistency before submitting.')
