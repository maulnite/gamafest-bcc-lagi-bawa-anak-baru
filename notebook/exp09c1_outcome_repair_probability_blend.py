# %% [markdown]
# # 00. EXP09C.1 — Ringkasan Eksperimen
#
# Bagian ini menjelaskan tujuan EXP09C.1. Eksperimen ini melanjutkan EXP09C yang menjadi best baru
# dengan gain kecil. Fokusnya bukan menambah model, tetapi mengatur seberapa kuat outcome repair
# memengaruhi decoder lewat probability blending.
...
# XGBoost/TabPFN/tqdm, dan output disimpan ke folder EXP09C.1.
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'exp09c1_outcome_repair_probability_blend'
...
# structure, cold-start flags, women temporal trend, dan confederation interaction. Pada EXP09C.1,
...
# %% [markdown]
# # 10. Train Baseline Outcome dan Repair Outcome
#
# Bagian ini melatih dua sumber outcome probability. Baseline memakai fitur EXP05A, repair memakai
# fitur EXP05A+EXP09A untuk outcome head, sedangkan goal/tail tetap baseline.
...
# For blending, raw_base is the anchor because goal and tail must remain baseline.
...
head_feature_config_df = pd.DataFrame([
    {'head': 'goal_a', 'source_for_blend': 'baseline', 'feature_set': 'EXP05A', 'n_features': len(BASE_HEAD_CONFIG['goal']['feature_cols']), 'n_cat_features': len(BASE_HEAD_CONFIG['goal']['cat_features'])},
    {'head': 'goal_b', 'source_for_blend': 'baseline', 'feature_set': 'EXP05A', 'n_features': len(BASE_HEAD_CONFIG['goal']['feature_cols']), 'n_cat_features': len(BASE_HEAD_CONFIG['goal']['cat_features'])},
    {'head': 'tail', 'source_for_blend': 'baseline', 'feature_set': 'EXP05A', 'n_features': len(BASE_HEAD_CONFIG['tail']['feature_cols']), 'n_cat_features': len(BASE_HEAD_CONFIG['tail']['cat_features'])},
    {'head': 'outcome_base', 'source_for_blend': 'baseline', 'feature_set': 'EXP05A', 'n_features': len(BASE_HEAD_CONFIG['outcome']['feature_cols']), 'n_cat_features': len(BASE_HEAD_CONFIG['outcome']['cat_features'])},
    {'head': 'outcome_repair', 'source_for_blend': 'repair', 'feature_set': 'EXP05A_PLUS_EXP09A', 'n_features': len(REPAIR_HEAD_CONFIG['outcome']['feature_cols']), 'n_cat_features': len(REPAIR_HEAD_CONFIG['outcome']['cat_features'])},
])
...
# %% [markdown]
# # 11. Build Raw Prediction untuk Blend
#
# Bagian ini menghasilkan raw prediction validation untuk baseline dan repair. raw_base menjadi anchor
# untuk goal/tail, sedangkan raw_repair hanya dipakai mengambil outcome probability repair.

# %%
log_section('Build raw prediction for probability blend')
...
# %% [markdown]
# # 12. Alpha Blend Decoder Grid
#
# Bagian ini mencoba alpha 0.00, 0.25, 0.50, 0.75, dan 1.00. Untuk setiap alpha, outcome probability
# di-blend lalu dievaluasi dengan decoder yang sama agar efek alpha bisa dibandingkan adil.

# %%
def blend_outcome_probabilities(raw_base, raw_repair, alpha):
    """
    Blend outcome probability only. Goal and tail signals are preserved from raw_base.
    raw_base and raw_repair must contain the same match_id set.
    """
    prob_cols = ['pred_outcome_proba_0', 'pred_outcome_proba_1', 'pred_outcome_proba_2']
    base = raw_base[['match_id'] + prob_cols].copy()
    repair = raw_repair[['match_id'] + prob_cols].copy()
    merged = base.merge(
        repair,
        on='match_id',
        how='inner',
    ...
    assert out[prob_cols].notna().all().all(), 'Blended probability has NaN'
    return out


def evaluate_alpha_prediction(alpha, raw_base, raw_repair, decoder_params, alpha0_detail=None):
    raw_alpha = blend_outcome_probabilities(raw_base, raw_repair, alpha)
    tail_prob = raw_base[['match_id'] + TAIL_COLS].copy()
    pred = decode_directional_tail_batch(
        raw_alpha.drop(columns=TAIL_COLS, errors='ignore'),
        tail_prob,
        decoder_params,
        scoreline_prior_lookup[int(decoder_params['MAX_GOALS'])],
    )
    pred_match = raw_alpha.merge(pred, on='match_id', how='left', validate='one_to_one')
    variant_name = f'alpha_{int(round(alpha * 100)):03d}'
    detail = prediction_detail_table(pred_match, variant_name)
    summary = summarize_prediction_detail(detail, variant_name)
    summary['alpha'] = float(alpha)

    if alpha0_detail is not None:
        compare = detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss', 'tournament_weight']].merge(
            alpha0_detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss']],
            on='match_id',
            how='left',
            suffixes=('', '_alpha0'),
            validate='one_to_one',
        ...
        changed = (
            (compare['pred_team_a_goals'] != compare['pred_team_a_goals_alpha0'])
            | (compare['pred_team_b_goals'] != compare['pred_team_b_goals_alpha0'])
        )
        delta = compare['official_loss'] - compare['official_loss_alpha0']
        summary['n_changed_vs_alpha_0'] = int(changed.sum())
        summary['changed_rate_vs_alpha_0'] = float(changed.mean())
        ...
        summary['weighted_loss_delta_vs_alpha_0'] = float((delta * compare['tournament_weight']).sum() / compare['tournament_weight'].sum())
    else:
        summary['n_changed_vs_alpha_0'] = 0
        summary['changed_rate_vs_alpha_0'] = 0.0
        summary['n_improved_changed'] = 0
        summary['n_worsened_changed'] = 0
        summary['n_neutral_changed'] = 0
        summary['weighted_loss_delta_vs_alpha_0'] = 0.0

    return raw_alpha, pred_match, detail, summary


log_section('Alpha blend decoder grid')

# First, tune decoder once on alpha=0.00 baseline. This keeps the decoder stable across alpha values.
tail_prob_base_valid = raw_base_valid[['match_id'] + TAIL_COLS].copy()
decoder_grid_base, selected_decoder, pred_alpha0 = tune_tail_aware_decoder_from_raw_outputs(
    raw_base_valid,
    tail_prob_base_valid,
    TAIL_GRID,
    scoreline_prior_lookup,
    top_n=3,
)
decoder_grid_base.to_csv(SUM_DIR / 'alpha_blend_grid.csv', index=False)
log_saved(SUM_DIR / 'alpha_blend_grid.csv')
...
for alpha in ALPHA_GRID:
    raw_alpha, pred_match, detail, summary = evaluate_alpha_prediction(
        alpha,
        raw_base_valid,
        raw_repair_valid,
        selected_decoder,
        alpha0_detail=alpha0_detail,
    )
    if float(alpha) == 0.0:
        alpha0_detail = detail.copy()
        # recompute alpha 0 with its own detail as reference so fields are consistent
        raw_alpha, pred_match, detail, summary = evaluate_alpha_prediction(
            alpha,
            raw_base_valid,
            raw_repair_valid,
    ...
    alpha_tag = f'alpha_{int(round(alpha * 100)):03d}'
    pred_path = PRED_DIR / f'valid_pred_match_alpha_{int(round(alpha * 100)):03d}.csv'
    pred_match.to_csv(pred_path, index=False)
    log_saved(pred_path)
    ALPHA_RESULTS[alpha_tag] = {
        'alpha': float(alpha),
        'raw': raw_alpha,
        'pred_match': pred_match,
        'detail': detail,
    ...
    alpha_metric_rows.append(summary)

alpha_blend_metrics_df = pd.DataFrame(alpha_metric_rows).sort_values('awmae').reset_index(drop=True)
alpha_blend_metrics_df.to_csv(SUM_DIR / 'alpha_blend_metrics.csv', index=False)
display(alpha_blend_metrics_df)
log_saved(SUM_DIR / 'alpha_blend_metrics.csv')

# Optional figures for quick visual inspection.
try:
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.plot(alpha_blend_metrics_df.sort_values('alpha')['alpha'], alpha_blend_metrics_df.sort_values('alpha')['awmae'], marker='o')
    ax.set_title('EXP09C.1 Alpha vs AW-MAE')
    ax.set_xlabel('alpha')
    ax.set_ylabel('AW-MAE')
    fig.tight_layout()
    fig.savefig(FIG_DIR / 'alpha_vs_awmae.png', dpi=160)
    plt.close(fig)
    log_saved(FIG_DIR / 'alpha_vs_awmae.png')
except Exception as e:
    log_warn(f'Failed to save alpha_vs_awmae figure: {repr(e)}')

# %% [markdown]
# # 13. Domain Error Analysis per Alpha
#
# Bagian ini mengecek performa setiap alpha di subgroup seperti gender, tournament_structure, dan
# blowout/high-total flags. Tujuannya melihat apakah alpha kecil memperbaiki area tertentu tanpa
# merusak subgroup lain.

# %%
log_section('Domain error analysis per alpha')

all_alpha_detail_df = pd.concat([v['detail'] for v in ALPHA_RESULTS.values()], ignore_index=True)

scoreline_dist = scoreline_distribution(all_alpha_detail_df)
scoreline_dist.to_csv(SUM_DIR / 'alpha_blend_scoreline_distribution.csv', index=False)
display(scoreline_dist.groupby('variant_name').head(10).reset_index(drop=True))
log_saved(SUM_DIR / 'alpha_blend_scoreline_distribution.csv')

def compute_domain_error_per_alpha(detail_df, group_col):
    if group_col not in detail_df.columns:
        return pd.DataFrame()
    rows = []
    for variant, g0 in detail_df.groupby('variant_name'):
        alpha_value = float(str(variant).replace('alpha_', '')) / 100.0 if str(variant).startswith('alpha_') else np.nan
        for key, g in g0.groupby(group_col, dropna=False):
            rows.append({
...
alpha_domain_gender = compute_domain_error_per_alpha(all_alpha_detail_df, 'gender')
alpha_domain_gender.to_csv(SUM_DIR / 'alpha_blend_domain_gender.csv', index=False)
display(alpha_domain_gender.head(20))
log_saved(SUM_DIR / 'alpha_blend_domain_gender.csv')

if 'tournament_structure' in all_alpha_detail_df.columns:
    alpha_domain_tournament = compute_domain_error_per_alpha(all_alpha_detail_df, 'tournament_structure')
else:
    alpha_domain_tournament = pd.DataFrame()
alpha_domain_tournament.to_csv(SUM_DIR / 'alpha_blend_domain_tournament_structure.csv', index=False)
display(alpha_domain_tournament.head(30))
log_saved(SUM_DIR / 'alpha_blend_domain_tournament_structure.csv')

blowout_detail = all_alpha_detail_df.copy()
blowout_detail['blowout_domain'] = np.select(
    [
        blowout_detail['is_blowout_7plus'].eq(1),
        blowout_detail['is_blowout_5plus'].eq(1),
        blowout_detail['is_high_total_6plus'].eq(1),
    ],
    ['is_blowout_7plus', 'is_blowout_5plus', 'is_high_total_6plus'],
    default='normal_or_low_total',
)
alpha_domain_blowout = compute_domain_error_per_alpha(blowout_detail, 'blowout_domain')
alpha_domain_blowout.to_csv(SUM_DIR / 'alpha_blend_domain_blowout.csv', index=False)
display(alpha_domain_blowout.head(30))
log_saved(SUM_DIR / 'alpha_blend_domain_blowout.csv')

try:
    plot_df = alpha_blend_metrics_df.sort_values('alpha')
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.plot(plot_df['alpha'], plot_df['outcome_rate'], marker='o', label='outcome_rate')
    ax.plot(plot_df['alpha'], plot_df['exact_rate'], marker='o', label='exact_rate')
    ax.plot(plot_df['alpha'], plot_df['gd_rate'], marker='o', label='gd_rate')
    ax.set_title('EXP09C.1 Alpha vs Exact/Outcome/GD')
    ax.set_xlabel('alpha')
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIG_DIR / 'alpha_vs_outcome_exact_gd.png', dpi=160)
    plt.close(fig)
    log_saved(FIG_DIR / 'alpha_vs_outcome_exact_gd.png')
except Exception as e:
    log_warn(f'Failed to save alpha metric figure: {repr(e)}')

# %% [markdown]
# # 14. Final Decision
#
# Bagian ini memilih alpha terbaik berdasarkan AW-MAE dan safety check. JSON tetap disimpan sebagai
# artifact, tetapi tampilan utama dibuat sebagai DataFrame agar rapi.

# %%
log_section('Final decision')

alpha_metrics = alpha_blend_metrics_df.copy()
baseline = alpha_metrics[alpha_metrics['alpha'].eq(0.0)].iloc[0].to_dict()
alpha_metrics['awmae_gain_vs_alpha0'] = float(baseline['awmae']) - alpha_metrics['awmae']
alpha_metrics['exact_delta_vs_alpha0'] = alpha_metrics['exact_rate'] - float(baseline['exact_rate'])
alpha_metrics['gd_delta_vs_alpha0'] = alpha_metrics['gd_rate'] - float(baseline['gd_rate'])
alpha_metrics['outcome_delta_vs_alpha0'] = alpha_metrics['outcome_rate'] - float(baseline['outcome_rate'])
alpha_metrics['top3_delta_vs_alpha0'] = alpha_metrics['top3_scoreline_share'] - float(baseline['top3_scoreline_share'])

alpha_metrics['safety_pass'] = (
    (alpha_metrics['awmae_gain_vs_alpha0'] > 0)
    & (alpha_metrics['exact_delta_vs_alpha0'] >= -0.010)
    & (alpha_metrics['gd_delta_vs_alpha0'] >= -0.015)
    & (alpha_metrics['top3_delta_vs_alpha0'] <= 0.030)
    & (alpha_metrics['mean_pred_total'].between(1.0, 4.5))
)

safe_candidates = alpha_metrics[alpha_metrics['safety_pass']].sort_values('awmae')
if len(safe_candidates):
    selected_row = safe_candidates.iloc[0].to_dict()
    selected_alpha = float(selected_row['alpha'])
    decision_code = 'A'
    decision_text = 'Alpha blend improves AW-MAE and passes safety checks.'
else:
    best_row = alpha_metrics.sort_values('awmae').iloc[0].to_dict()
    if float(best_row['alpha']) == 0.0:
        decision_code = 'C'
        decision_text = 'Baseline alpha 0 remains best.'
    elif float(best_row['alpha']) == 1.0:
        decision_code = 'D'
        decision_text = 'Full repair alpha 1 is best by AW-MAE but unsafe, fallback to baseline alpha 0.'
    elif (alpha_metrics['outcome_delta_vs_alpha0'] > 0).any() and (alpha_metrics['awmae_gain_vs_alpha0'] <= 0).all():
        decision_code = 'B'
        decision_text = 'Some alpha improves outcome signal but not AW-MAE, fallback to baseline alpha 0.'
    else:
        decision_code = 'C'
        decision_text = 'No alpha blend passes safety. Final selected alpha is baseline alpha 0.'
    selected_row = alpha_metrics[alpha_metrics['alpha'].eq(0.0)].iloc[0].to_dict()
    selected_alpha = 0.0

selected_alpha_config = {
    'experiment': 'EXP09C.1',
    'selected_alpha': selected_alpha,
    'selected_decoder': selected_decoder,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'baseline_alpha0_awmae': float(baseline['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_alpha0': float(selected_row.get('awmae_gain_vs_alpha0', 0.0)),
}

with open(SUM_DIR / 'selected_alpha_config.json', 'w', encoding='utf-8') as f:
    json.dump(selected_alpha_config, f, indent=2, ensure_ascii=False)

selected_alpha_config_df = pd.DataFrame([{'key': k, 'value': str(v)} for k, v in selected_alpha_config.items()])
display(alpha_metrics.sort_values('awmae').reset_index(drop=True))
display(selected_alpha_config_df)
log_saved(SUM_DIR / 'selected_alpha_config.json')

final_decision = {
    'experiment': 'EXP09C.1',
    'base_pipeline': 'EXP05A-LITE-FIX-V2 / EXP09C',
    'main_change': 'outcome probability blend between baseline and repair outcome heads',
    'selected_alpha': selected_alpha,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'baseline_alpha0_awmae': float(baseline['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_alpha0': float(selected_row.get('awmae_gain_vs_alpha0', 0.0)),
    'caveat': 'Goal and tail predictions stay baseline. Only outcome probability is blended before decoder.',
}
...
# %% [markdown]
# # 15. Final Model Training dan Test Inference
#
# Bagian ini melatih model final pada full train sesuai selected alpha. Test inference memakai
# goal/tail baseline dan outcome probability blend sesuai alpha terpilih.
...
# Train baseline and repair models on the full training feature frame.
# Goal/tail come from full_base_models. Outcome repair probability comes from full_repair_models.
...
test_raw_blend = blend_outcome_probabilities(test_raw_base, test_raw_repair, selected_alpha)

test_pred = decode_directional_tail_batch(
    test_raw_blend.drop(columns=TAIL_COLS, errors='ignore'),
...
test_pred_match_best['selected_alpha'] = selected_alpha
test_pred_match_best['selected_variant'] = f'alpha_{int(round(selected_alpha * 100)):03d}'
test_pred_match_best.to_csv(PRED_DIR / 'test_pred_exp09c1_best_safe.csv', index=False)
log_saved(PRED_DIR / 'test_pred_exp09c1_best_safe.csv')

print('[TEST INFERENCE SUMMARY]', flush=True)
log_info(f'selected_alpha: {selected_alpha}')
...
# %% [markdown]
# # 16. Submission Mapping dan Pair Consistency
#
...
submission_path = SUB_DIR / 'submission_exp09c1_best_safe.csv'
...
# %% [markdown]
# # 17. Runtime Summary dan Catatan Akhir
#
# Bagian ini menyimpan runtime summary dan menampilkan ringkasan akhir eksperimen. Output utama yang
# perlu dicek adalah selected alpha, decision code, dan path submission final.
...
closing_summary_df = pd.DataFrame([
    {'item': 'experiment', 'value': 'EXP09C.1'},
    {'item': 'selected_alpha', 'value': str(selected_alpha)},
    {'item': 'decision_code', 'value': decision_code},
    {'item': 'submission_path', 'value': str(submission_path)},
])
display(closing_summary_df)
