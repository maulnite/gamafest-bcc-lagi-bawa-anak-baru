# %% [markdown]
# # 00. EXP10A — Ringkasan Eksperimen
#
# Bagian ini menjelaskan tujuan EXP10A. Eksperimen ini melanjutkan EXP09D yang menjadi best terbaru.
# Fokusnya bukan mengganti model utama, tetapi menguji Poisson median dan goal decoder shrinkage
# pada anchor skor yang dipakai direct cost decoder.
...
# XGBoost/TabPFN/tqdm, dan output disimpan ke folder EXP10A.
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'exp10a_poisson_median_goal_decoder_shrinkage'
...
log_info('EXP10A source of truth: EXP05A-LITE-FIX-V2 with EXP09D as current best mainline. EXP09A repair is used only for outcome repair functions.')
...
# probability outcome repair dipakai berdasarkan tournament_structure. EXP10A memakai gate terpilih
...
# %% [markdown]
# # 13. EXP09D Gate Selection sebagai Baseline EXP10A
#
...
log_section('EXP09D gate selection baseline for EXP10A')
GATE_RESULTS = {}
gate_metric_rows = []
v0_detail = None
...
selected_gate_df.to_csv(SUM_DIR / 'exp09d_selected_gate_for_exp10a.csv', index=False)
log_saved(SUM_DIR / 'exp09d_selected_gate_for_exp10a.csv')

# %% [markdown]
# # 14. Poisson Median dan Goal Anchor Transform
#
# Bagian ini membuat fungsi Poisson median tanpa scipy, prior total historis, dan transform anchor goal.
# Anchor baru disimpan di `decode_goal_a_anchor` dan `decode_goal_b_anchor`, sehingga decoder lama tetap
# kompatibel dan hanya direct cost-nya yang membaca kolom anchor jika tersedia.

# %%
def poisson_median_scalar(lam, max_k=30):
    lam = float(np.clip(lam, 1e-9, max_k))
    p = math.exp(-lam)
    cdf = p
    if cdf >= 0.5:
        return 0
...
def poisson_median_array(lam_arr, max_k=30):
    return np.array([poisson_median_scalar(x, max_k=max_k) for x in lam_arr], dtype=float)


def _goal_cols_for_prior(df):
    candidates = [
        ('team_a_goals', 'team_b_goals'),
        ('actual_team_a_goals', 'actual_team_b_goals'),
        ('y_goal_a', 'y_goal_b'),
        ('goal_a', 'goal_b'),
    ]
    for a_col, b_col in candidates:
        if a_col in df.columns and b_col in df.columns:
            return a_col, b_col
    raise KeyError('No goal columns found for prior total lookup')


def build_prior_total_lookup(train_df):
    df = train_df.copy()
    a_col, b_col = _goal_cols_for_prior(df)
    df['true_total_for_prior'] = pd.to_numeric(df[a_col], errors='coerce') + pd.to_numeric(df[b_col], errors='coerce')
    lookup = {}
    if 'tournament_structure' in df.columns and 'gender' in df.columns:
        for key, g in df.groupby(['gender', 'tournament_structure'], dropna=False):
            if g['true_total_for_prior'].notna().any():
                lookup[('gender_structure', str(key[0]), str(key[1]))] = float(g['true_total_for_prior'].mean())
    if 'gender' in df.columns:
        for key, g in df.groupby(['gender'], dropna=False):
            if g['true_total_for_prior'].notna().any():
                lookup[('gender', str(key))] = float(g['true_total_for_prior'].mean())
    lookup[('global',)] = float(df['true_total_for_prior'].mean()) if df['true_total_for_prior'].notna().any() else 2.5
    return lookup


def get_prior_total(row, lookup):
    gender = str(row.get('gender', '__MISSING__'))
    ts = str(row.get('tournament_structure', '__MISSING__'))
    key = ('gender_structure', gender, ts)
    if key in lookup:
        return lookup[key]
    key = ('gender', gender)
    if key in lookup:
        return lookup[key]
    return lookup.get(('global',), 2.5)


def apply_goal_anchor_transform(raw_df, config, prior_total_lookup=None):
    out = raw_df.copy()
    a_raw = pd.to_numeric(out['pred_goal_a_cont'], errors='coerce').fillna(1.0).clip(0, 30).to_numpy(float)
    b_raw = pd.to_numeric(out['pred_goal_b_cont'], errors='coerce').fillna(1.0).clip(0, 30).to_numpy(float)

    mode = str(config.get('mode', 'original'))
    beta = float(config.get('median_beta', 0.0))
    shrink = float(config.get('total_shrink', 0.0))
    a_anchor = a_raw.copy()
    b_anchor = b_raw.copy()

    if mode in ['poisson_median', 'median_blend', 'median_total_shrink']:
        a_med = poisson_median_array(a_raw, max_k=int(config.get('poisson_max_k', 30)))
        b_med = poisson_median_array(b_raw, max_k=int(config.get('poisson_max_k', 30)))
        if mode == 'poisson_median':
            beta = 1.0
        a_anchor = (1 - beta) * a_raw + beta * a_med
        b_anchor = (1 - beta) * b_raw + beta * b_med

    if mode in ['prior_total_shrink', 'median_total_shrink']:
        if prior_total_lookup is None:
            raise ValueError('prior_total_lookup is required for total shrink mode')
        prior_total = np.array([get_prior_total(row, prior_total_lookup) for _, row in out.iterrows()], dtype=float)
        raw_total = a_anchor + b_anchor
        raw_diff = a_anchor - b_anchor
        shrunk_total = (1 - shrink) * raw_total + shrink * prior_total
        a_anchor = (shrunk_total + raw_diff) / 2
        b_anchor = (shrunk_total - raw_diff) / 2

    out['decode_goal_a_anchor'] = np.clip(a_anchor, 0, 30)
    out['decode_goal_b_anchor'] = np.clip(b_anchor, 0, 30)
    out['anchor_mode'] = mode
    out['anchor_median_beta'] = beta
    out['anchor_total_shrink'] = shrink
    return out


ANCHOR_CONFIGS = [
    {'name': 'D0_exp09d_original', 'mode': 'original', 'median_beta': 0.0, 'total_shrink': 0.0},
    {'name': 'D1_poisson_median_full', 'mode': 'poisson_median', 'median_beta': 1.0, 'total_shrink': 0.0},
    {'name': 'D2_median_blend_beta025', 'mode': 'median_blend', 'median_beta': 0.25, 'total_shrink': 0.0},
    {'name': 'D2_median_blend_beta050', 'mode': 'median_blend', 'median_beta': 0.50, 'total_shrink': 0.0},
    {'name': 'D2_median_blend_beta075', 'mode': 'median_blend', 'median_beta': 0.75, 'total_shrink': 0.0},
    {'name': 'D3_total_shrink_010', 'mode': 'prior_total_shrink', 'median_beta': 0.0, 'total_shrink': 0.10},
    {'name': 'D3_total_shrink_020', 'mode': 'prior_total_shrink', 'median_beta': 0.0, 'total_shrink': 0.20},
    {'name': 'D3_total_shrink_030', 'mode': 'prior_total_shrink', 'median_beta': 0.0, 'total_shrink': 0.30},
    {'name': 'D4_median050_totalshrink020', 'mode': 'median_total_shrink', 'median_beta': 0.50, 'total_shrink': 0.20},
]

with open(SUM_DIR / 'anchor_variant_config.json', 'w', encoding='utf-8') as f:
    json.dump(ANCHOR_CONFIGS, f, indent=2, ensure_ascii=False)
anchor_config_df = pd.DataFrame(ANCHOR_CONFIGS)
display(anchor_config_df)
log_saved(SUM_DIR / 'anchor_variant_config.json')

prior_total_lookup_valid = build_prior_total_lookup(train_sup_full)
log_info(f'prior_total_lookup keys: {len(prior_total_lookup_valid):,}')

# %% [markdown]
# # 15. Anchor Screening Cepat
#
# Bagian ini mengevaluasi semua anchor config memakai decoder EXP09D terpilih. Tahap ini cepat karena
# belum men-tune decoder ulang untuk setiap anchor. Outputnya adalah tabel screening dan prediksi validasi
# untuk beberapa anchor utama.

# %%
def add_change_metrics_vs_baseline(summary, detail, baseline_detail):
    compare = detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss', 'tournament_weight']].merge(
        baseline_detail[['match_id', 'pred_team_a_goals', 'pred_team_b_goals', 'official_loss']],
        on='match_id',
        how='left',
    ...
    changed = (
        (compare['pred_team_a_goals'] != compare['pred_team_a_goals_d0'])
        | (compare['pred_team_b_goals'] != compare['pred_team_b_goals_d0'])
    )
    delta = compare['official_loss'] - compare['official_loss_d0']
    summary['n_changed_vs_d0'] = int(changed.sum())
    summary['changed_rate_vs_d0'] = float(changed.mean())
    summary['n_improved_changed'] = int(((delta < 0) & changed).sum())
    summary['n_worsened_changed'] = int(((delta > 0) & changed).sum())
    summary['n_neutral_changed'] = int(((delta == 0) & changed).sum())
    summary['weighted_loss_delta_vs_d0'] = float((delta * compare['tournament_weight']).sum() / compare['tournament_weight'].sum())
    return summary


def decode_anchor_config(raw_gate, tail_prob, config, decoder_params, prior_lookup, variant_name):
    raw_anchor = apply_goal_anchor_transform(raw_gate, config, prior_lookup)
    pred = decode_directional_tail_batch(
        raw_anchor.drop(columns=TAIL_COLS, errors='ignore'),
        tail_prob,
        decoder_params,
        scoreline_prior_lookup[int(decoder_params['MAX_GOALS'])],
    )
    pred_match = raw_anchor.merge(pred, on='match_id', how='left', validate='one_to_one')
    detail = prediction_detail_table(pred_match, variant_name)
    summary = summarize_prediction_detail(detail, variant_name)
    summary.update({
        'anchor_mode': config.get('mode', 'original'),
        'median_beta': float(config.get('median_beta', 0.0)),
        'total_shrink': float(config.get('total_shrink', 0.0)),
        'decoder_params': str(decoder_params),
    })
    return raw_anchor, pred_match, detail, summary


log_section('Anchor screening with selected EXP09D decoder')
tail_prob_valid = raw_base_valid[['match_id'] + TAIL_COLS].copy()
screening_results = {}
screen_rows = []
d0_screen_detail = None

for cfg in ANCHOR_CONFIGS:
    name = cfg['name']
    raw_anchor, pred_match, detail, summary = decode_anchor_config(
        raw_gate_valid,
        tail_prob_valid,
        cfg,
        selected_exp09d_decoder,
        prior_total_lookup_valid,
        name,
    )
    if name == 'D0_exp09d_original':
        d0_screen_detail = detail.copy()
        summary.update({
            'n_changed_vs_d0': 0,
            'changed_rate_vs_d0': 0.0,
            'n_improved_changed': 0,
            'n_worsened_changed': 0,
            'n_neutral_changed': 0,
            'weighted_loss_delta_vs_d0': 0.0,
        })
    else:
        summary = add_change_metrics_vs_baseline(summary, detail, d0_screen_detail)

    screening_results[name] = {'raw_anchor': raw_anchor, 'pred_match': pred_match, 'detail': detail, 'summary': summary, 'config': cfg}
    screen_rows.append(summary)

    if name in ['D0_exp09d_original', 'D1_poisson_median_full']:
        filename = 'valid_pred_match_d0_exp09d_original.csv' if name == 'D0_exp09d_original' else 'valid_pred_match_d1_poisson_median.csv'
        pred_match.to_csv(PRED_DIR / filename, index=False)
        log_saved(PRED_DIR / filename)

anchor_screening_df = pd.DataFrame(screen_rows).sort_values('awmae').reset_index(drop=True)
anchor_screening_df.to_csv(SUM_DIR / 'anchor_variant_screening.csv', index=False)
display(anchor_screening_df)
log_saved(SUM_DIR / 'anchor_variant_screening.csv')

# Save best D2 and D3 screening predictions for traceability.
for prefix_name, filename in [('D2_', 'valid_pred_match_d2_median_blend_best.csv'), ('D3_', 'valid_pred_match_d3_total_shrink_best.csv')]:
    subset = anchor_screening_df[anchor_screening_df['variant_name'].str.startswith(prefix_name)]
    if len(subset):
        best_name = str(subset.sort_values('awmae').iloc[0]['variant_name'])
        screening_results[best_name]['pred_match'].to_csv(PRED_DIR / filename, index=False)
        log_saved(PRED_DIR / filename)

# %% [markdown]
# # 16. Anchor Refine dengan Decoder Retuning
#
# Bagian ini mengambil D0 original dan top 3 anchor selain D0 dari screening. Setiap anchor kemudian
# mendapat decoder tuning sendiri agar comparison lebih fair. Hasil refine menjadi dasar final decision.

# %%
def refine_anchor_config(config, variant_name):
    raw_anchor = apply_goal_anchor_transform(raw_gate_valid, config, prior_total_lookup_valid)
    decoder_grid, best_decoder, pred = tune_tail_aware_decoder_from_raw_outputs(
        raw_anchor,
        tail_prob_valid,
        TAIL_GRID,
    ...
    summary.update({
        ...
        'median_beta': float(config.get('median_beta', 0.0)),
        'total_shrink': float(config.get('total_shrink', 0.0)),
        'best_decoder': str(best_decoder),
    })
    return raw_anchor, pred_match, detail, summary, decoder_grid, best_decoder


log_section('Anchor refine with per-anchor decoder retuning')
d0_cfg = [c for c in ANCHOR_CONFIGS if c['name'] == 'D0_exp09d_original'][0]
top_non_d0_names = (
    anchor_screening_df[~anchor_screening_df['variant_name'].eq('D0_exp09d_original')]
    .sort_values('awmae')
    .head(3)['variant_name']
    .astype(str)
    .tolist()
)
refine_configs = [d0_cfg] + [copy.deepcopy(next(c for c in ANCHOR_CONFIGS if c['name'] == name)) for name in top_non_d0_names]

REFINE_RESULTS = {}
refine_rows = []
d0_refine_detail = None

for cfg in refine_configs:
    name = cfg['name']
    raw_anchor, pred_match, detail, summary, decoder_grid, best_decoder = refine_anchor_config(cfg, name)
    if name == 'D0_exp09d_original':
        d0_refine_detail = detail.copy()
        summary.update({
            'n_changed_vs_d0': 0,
            'changed_rate_vs_d0': 0.0,
            'n_improved_changed': 0,
        ...
        summary = add_change_metrics_vs_baseline(summary, detail, d0_refine_detail)

    safe_name = name.replace('/', '_').replace(' ', '_')
    pred_match.to_csv(PRED_DIR / f'valid_pred_match_{safe_name}.csv', index=False)
    decoder_grid.to_csv(SUM_DIR / f'decoder_grid_{safe_name}.csv', index=False)
    log_saved(PRED_DIR / f'valid_pred_match_{safe_name}.csv')
    log_saved(SUM_DIR / f'decoder_grid_{safe_name}.csv')
...
anchor_refine_metrics_df = pd.DataFrame(refine_rows).sort_values('awmae').reset_index(drop=True)
anchor_refine_metrics_df.to_csv(SUM_DIR / 'anchor_variant_refine_metrics.csv', index=False)
display(anchor_refine_metrics_df)
log_saved(SUM_DIR / 'anchor_variant_refine_metrics.csv')

# %% [markdown]
# # 17. Domain Error Analysis
#
# Bagian ini mengecek performa anchor refine berdasarkan gender, tournament_structure, blowout,
# dan high-total flags. Tujuannya melihat apakah Poisson median/shrinkage membantu magnitude gol
# tanpa merusak subgroup tertentu.

# %%
log_section('Domain error analysis for anchor variants')
all_refine_detail_df = pd.concat([v['detail'] for v in REFINE_RESULTS.values()], ignore_index=True)

domain_gender_df = compute_domain_error_per_variant(all_refine_detail_df, 'gender')
domain_tournament_df = compute_domain_error_per_variant(all_refine_detail_df, 'tournament_structure')
domain_blowout_df = pd.concat([
    compute_domain_error_per_variant(all_refine_detail_df, 'is_blowout_5plus').assign(domain='is_blowout_5plus'),
    compute_domain_error_per_variant(all_refine_detail_df, 'is_blowout_7plus').assign(domain='is_blowout_7plus'),
    compute_domain_error_per_variant(all_refine_detail_df, 'is_high_total_6plus').assign(domain='is_high_total_6plus'),
], ignore_index=True)
scoreline_dist_df = scoreline_distribution(all_refine_detail_df)
...
display(domain_gender_df.head(20))
display(domain_tournament_df.head(30))
display(scoreline_dist_df.groupby('variant_name').head(5).reset_index(drop=True))
...
# # 18. Final Decision
#
# Bagian ini memilih anchor terbaik berdasarkan AW-MAE dan safety check. Jika anchor baru tidak lolos,
# final akan fallback ke D0 EXP09D original. JSON tetap disimpan, tetapi tampilan notebook memakai DataFrame.

# %%
log_section('Final decision for EXP10A')
metrics = anchor_refine_metrics_df.copy()
d0 = metrics[metrics['variant_name'].eq('D0_exp09d_original')].iloc[0]
metrics['awmae_gain_vs_d0'] = float(d0['awmae']) - metrics['awmae']

safe_candidates = []
for _, row in metrics[~metrics['variant_name'].eq('D0_exp09d_original')].iterrows():
    safe = (
        (float(row['awmae']) < float(d0['awmae']))
        and (float(row['base_mae']) <= float(d0['base_mae']) + 1e-12)
        and (float(row['exact_rate']) >= float(d0['exact_rate']) - 0.006)
        and (float(row['outcome_rate']) >= float(d0['outcome_rate']) - 0.008)
        and (float(row['gd_rate']) >= float(d0['gd_rate']) - 0.010)
        and (float(row['top3_scoreline_share']) <= float(d0['top3_scoreline_share']) + 0.025)
        and (1.0 <= float(row['mean_pred_total']) <= 4.5)
    )
    if safe:
...
if safe_candidates:
    ...
    selected_anchor_name = str(selected_row['variant_name'])
    decision_code = 'A'
    decision_text = 'Anchor shrinkage improves AW-MAE and passes safety checks.'
else:
    best_non_d0 = metrics[~metrics['variant_name'].eq('D0_exp09d_original')].sort_values('awmae').iloc[0]
    if float(best_non_d0['base_mae']) < float(d0['base_mae']) and float(best_non_d0['awmae']) >= float(d0['awmae']):
        decision_code = 'B'
        decision_text = 'A new anchor improves base MAE but not AW-MAE, so D0 remains selected.'
    elif str(metrics.sort_values('awmae').iloc[0]['variant_name']) == 'D0_exp09d_original':
        decision_code = 'C'
        decision_text = 'D0 EXP09D original remains best.'
    else:
        decision_code = 'D'
        decision_text = 'A new anchor has best AW-MAE but fails safety checks, so fallback to D0.'
    selected_anchor_name = 'D0_exp09d_original'
    selected_row = metrics[metrics['variant_name'].eq('D0_exp09d_original')].iloc[0]

selected_anchor_config = copy.deepcopy(REFINE_RESULTS[selected_anchor_name]['config'])
selected_anchor_decoder = REFINE_RESULTS[selected_anchor_name]['best_decoder']

selected_anchor_json = {
    'experiment': 'EXP10A',
    'base_pipeline': 'EXP05A-LITE-FIX-V2 / EXP09D',
    'selected_gate': selected_gate_name,
    'selected_anchor': selected_anchor_name,
    'selected_anchor_config': selected_anchor_config,
    'selected_decoder': selected_anchor_decoder,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'd0_awmae': float(d0['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_d0': float(float(d0['awmae']) - float(selected_row['awmae'])),
}
with open(SUM_DIR / 'selected_anchor_config.json', 'w', encoding='utf-8') as f:
    json.dump(selected_anchor_json, f, indent=2, ensure_ascii=False)

final_decision = {
    'experiment': 'EXP10A',
    'main_change': 'Poisson median and goal decoder shrinkage over EXP09D tournament-gated outcome repair',
    'selected_gate': selected_gate_name,
    'selected_anchor': selected_anchor_name,
    'decision_code': decision_code,
    'decision_text': decision_text,
    ...
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_d0': float(float(d0['awmae']) - float(selected_row['awmae'])),
    'caveat': 'Goal/outcome/tail models stay EXP09D. Only decoder goal anchor is transformed.',
}
...
selected_anchor_config_df = pd.DataFrame([{'key': k, 'value': str(v)} for k, v in selected_anchor_json.items()])
...
display(selected_anchor_config_df)
display(final_decision_df)
log_saved(SUM_DIR / 'selected_anchor_config.json')
log_saved(SUM_DIR / 'final_decision.json')
log_saved(SUM_DIR / 'final_decision_table.csv')

# %% [markdown]
# # 19. Final Model Training dan Test Inference
#
# Bagian ini melatih model final pada full train seperti EXP09D, membangun raw_gate test, menerapkan
# selected anchor transform EXP10A, lalu decode memakai selected decoder. Jika D0 terpilih, output
# secara konsep kembali ke EXP09D original.
...
full_base_models = train_final_gender_models_by_head_config(
    full_train_feature_df,
    valid_sup_full,
    final_selected,
    BASE_HEAD_CONFIG,
    1200,
)
full_repair_models = train_final_gender_models_by_head_config(
    full_train_feature_df,
    valid_sup_full,
    final_selected,
    REPAIR_HEAD_CONFIG,
...
test_raw_gate = apply_tournament_gated_outcome(
    test_raw_base,
    test_raw_repair,
    selected_gate_cfg['gate_map'],
    default_alpha=selected_gate_cfg['default_alpha'],
)

prior_total_lookup_full = build_prior_total_lookup(full_train_feature_df)
test_raw_anchor = apply_goal_anchor_transform(test_raw_gate, selected_anchor_config, prior_total_lookup_full)

test_pred = decode_directional_tail_batch(
    test_raw_anchor.drop(columns=TAIL_COLS, errors='ignore'),
    test_raw_base[['match_id'] + TAIL_COLS],
    selected_anchor_decoder,
    scoreline_prior_lookup[int(selected_anchor_decoder['MAX_GOALS'])],
)
...
test_pred_match_best['selected_anchor'] = selected_anchor_name
test_pred_match_best['mean_gate_alpha'] = float(test_raw_gate['gate_alpha'].mean())
test_pred_match_best.to_csv(PRED_DIR / 'test_pred_exp10a_best_safe.csv', index=False)
log_saved(PRED_DIR / 'test_pred_exp10a_best_safe.csv')

print('[TEST INFERENCE SUMMARY]', flush=True)
log_info(f'selected_gate: {selected_gate_name}')
log_info(f'selected_anchor: {selected_anchor_name}')
log_info(f'n_test_matches: {len(test_pred_match_best):,}')
log_info(f"mean_gate_alpha: {float(test_raw_gate['gate_alpha'].mean()):.4f}")
...
# %% [markdown]
# # 20. Submission Mapping dan Pair Consistency
#
# Bagian ini mengubah prediksi match-level kembali ke row-level submission. Validasi wajib mencakup
# shape, urutan Id, integer non-negative goals, no missing, no duplicate Id, dan pair consistency.

# %%
log_section('Submission mapping and pair consistency')
...
submission_path = SUB_DIR / 'submission_exp10a_best_safe.csv'
...
# %% [markdown]
# # 21. Runtime Summary dan Catatan Akhir
#
# Bagian ini menyimpan runtime summary dan menampilkan ringkasan akhir eksperimen. Output akhir harus
# jelas: selected gate, selected anchor, decision code, validation metric, dan path submission.

# %%
log_section('Runtime summary and closing notes')
SECTION_TIMES['total_runtime'] = time.time() - NOTEBOOK_START_TIME
...
closing_df = pd.DataFrame([
    {'item': 'experiment', 'value': 'EXP10A'},
    {'item': 'selected_gate', 'value': selected_gate_name},
    {'item': 'selected_anchor', 'value': selected_anchor_name},
    {'item': 'decision_code', 'value': decision_code},
    {'item': 'submission_path', 'value': str(submission_path)},
])
display(closing_df)
log_result('EXP10A finished. Review anchor_variant_refine_metrics.csv and final_decision_table.csv before trusting GT transfer.')
