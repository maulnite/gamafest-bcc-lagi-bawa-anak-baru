# %% [markdown]
# # 00. EXP09D — Ringkasan Eksperimen
#
# Bagian ini menjelaskan tujuan EXP09D. Eksperimen ini melanjutkan EXP09C dan EXP09C.1.
# Fokusnya bukan menambah model atau fitur baru, tetapi memakai outcome repair secara selektif
# berdasarkan tournament_structure agar repair tidak merusak struktur turnamen tertentu.
...
# XGBoost/TabPFN/tqdm, dan output disimpan ke folder EXP09D.
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'exp09d_tournament_gated_outcome_repair'
...
RUN_DATA_DRIVEN_GATE = False  # optional V4 is disabled by default to avoid validation overfit
...
with open(SUM_DIR / 'exp09d_feature_sets.json', 'w', encoding='utf-8') as f:
    json.dump(exp09d_feature_sets, f, indent=2, ensure_ascii=False)
# Compatibility copy of patch list under EXP09C name.
if 'EXP09A_PATCH_GROUPS' in globals():
    with open(SUM_DIR / 'exp09d_feature_patch_list.json', 'w', encoding='utf-8') as f:
        json.dump(EXP09A_PATCH_GROUPS, f, indent=2, ensure_ascii=False)
...
log_saved(SUM_DIR / 'exp09d_feature_sets.json')
...
# fitur EXP05A+EXP09A untuk outcome head, sedangkan goal/tail tetap baseline EXP05A.
...
# %% [markdown]
# # 11. Build Raw Prediction untuk Tournament Gate
#
...
# Output ini menjadi bahan gate tournament di section berikutnya.
...
# %% [markdown]
# # 12. Tournament-Gated Outcome Probability
#
# Bagian ini membuat gate variant untuk outcome probability. raw_base tetap menjadi anchor untuk goal
# dan tail, sedangkan outcome probability bisa berasal dari baseline, repair penuh, atau campuran
# berdasarkan tournament_structure. Gate disimpan sebagai tabel agar mudah dicek sebelum decode.
...
# %% [markdown]
# # 13. Decode dan Evaluate Gate Variants
#
# Bagian ini menjalankan decoder untuk setiap gate variant. Setiap variant men-tune decoder-nya sendiri
# berdasarkan outcome probability yang sudah digate, supaya V1 full repair benar-benar merepresentasikan
# behavior EXP09C dan V2/V3 dinilai secara fair. Metric utama yang dihitung adalah AW-MAE, base MAE,
# exact rate, outcome rate, GD rate, bias, mean predicted total, dan top-3 scoreline share.

# %%
def evaluate_gate_prediction(variant_name, raw_base, raw_repair, gate_cfg, baseline_detail=None):
    """Apply a tournament gate, tune decoder for that gated raw output, then evaluate.

    Decoder tuning is intentionally per variant. If V1/V2/V3 reused a decoder tuned only on V0,
    the comparison could be misleading because the outcome probability distribution entering the
...
log_section('Decode and evaluate tournament gate variants')
...
for variant_name, cfg in GATE_CONFIGS.items():
    ...
    save_map = {
        'V0_baseline_outcome': 'valid_pred_match_v0_baseline_outcome.csv',
        'V1_full_repair': 'valid_pred_match_v1_full_repair.csv',
        'V2_hard_helpful_gate': 'valid_pred_match_v2_hard_gate.csv',
        'V3_conservative_soft_gate': 'valid_pred_match_v3_soft_gate.csv',
    }
    pred_path = PRED_DIR / save_map.get(variant_name, f'valid_pred_match_{variant_name}.csv')
    pred_match.to_csv(pred_path, index=False)
    log_saved(pred_path)

    decoder_grid_path = SUM_DIR / f'decoder_grid_{variant_name}.csv'
...
gate_variant_metrics_df = pd.DataFrame(gate_metric_rows).sort_values('awmae').reset_index(drop=True)
gate_variant_metrics_df.to_csv(SUM_DIR / 'gate_variant_metrics.csv', index=False)
display(gate_variant_metrics_df)
log_saved(SUM_DIR / 'gate_variant_metrics.csv')

scoreline_dist = scoreline_distribution(pd.concat([v['detail'] for v in GATE_RESULTS.values()], ignore_index=True))
...
try:
    ...
    plot_df = gate_variant_metrics_df.sort_values('awmae')
    ax.bar(plot_df['variant_name'], plot_df['awmae'])
    ax.set_title('EXP09D Gate Variant AW-MAE')
    ax.set_ylabel('AW-MAE')
    ax.tick_params(axis='x', rotation=25)
    fig.tight_layout()
    fig.savefig(FIG_DIR / 'gate_variant_awmae.png', dpi=160)
    plt.close(fig)
    log_saved(FIG_DIR / 'gate_variant_awmae.png')
except Exception as e:
    log_warn(f'Failed to save gate_variant_awmae figure: {repr(e)}')

# %% [markdown]
# # 14. Domain Error Analysis
#
# Bagian ini mengecek performa tiap gate variant berdasarkan gender, tournament_structure, dan
# blowout/high-total flags. Tabel tournament_structure juga diberi delta terhadap V0 agar terlihat
# apakah gate mengurangi kerusakan di qualifier dan other_competitive.

# %%
log_section('Domain error analysis for gate variants')

all_gate_detail_df = pd.concat([v['detail'] for v in GATE_RESULTS.values()], ignore_index=True)
...
domain_gender = compute_domain_error_per_variant(all_gate_detail_df, 'gender')
domain_gender.to_csv(SUM_DIR / 'domain_error_gender.csv', index=False)
display(domain_gender.head(30))
log_saved(SUM_DIR / 'domain_error_gender.csv')

if 'tournament_structure' in all_gate_detail_df.columns:
    domain_tournament = compute_domain_error_per_variant(all_gate_detail_df, 'tournament_structure')
    v0_tour = domain_tournament[domain_tournament['variant_name'].eq('V0_baseline_outcome')][['tournament_structure', 'awmae', 'n']].rename(columns={'awmae': 'V0_awmae', 'n': 'V0_n'})
    domain_tournament = domain_tournament.merge(v0_tour, on='tournament_structure', how='left', validate='many_to_one')
    domain_tournament['delta_vs_v0'] = domain_tournament['awmae'] - domain_tournament['V0_awmae']
else:
    domain_tournament = pd.DataFrame()
domain_tournament.to_csv(SUM_DIR / 'domain_error_tournament_structure.csv', index=False)
display(domain_tournament.sort_values(['tournament_structure', 'variant_name']).head(60) if len(domain_tournament) else domain_tournament)
log_saved(SUM_DIR / 'domain_error_tournament_structure.csv')

blowout_detail = all_gate_detail_df.copy()
...
domain_blowout = compute_domain_error_per_variant(blowout_detail, 'blowout_domain')
domain_blowout.to_csv(SUM_DIR / 'domain_error_blowout.csv', index=False)
display(domain_blowout.head(40))
log_saved(SUM_DIR / 'domain_error_blowout.csv')

# %% [markdown]
# # 15. Final Decision
#
# Bagian ini memilih gate terbaik berdasarkan AW-MAE dan safety check. JSON tetap disimpan untuk
# artifact, tetapi tampilan utama dibuat sebagai DataFrame supaya rapi dan tidak berupa raw dict panjang.

# %%
log_section('Final decision')

metrics = gate_variant_metrics_df.copy()
v0 = metrics[metrics['variant_name'].eq('V0_baseline_outcome')].iloc[0].to_dict()
v1 = metrics[metrics['variant_name'].eq('V1_full_repair')].iloc[0].to_dict()
metrics['awmae_gain_vs_v0'] = float(v0['awmae']) - metrics['awmae']
metrics['exact_delta_vs_v0'] = metrics['exact_rate'] - float(v0['exact_rate'])
metrics['gd_delta_vs_v0'] = metrics['gd_rate'] - float(v0['gd_rate'])
metrics['outcome_delta_vs_v0'] = metrics['outcome_rate'] - float(v0['outcome_rate'])
metrics['top3_delta_vs_v0'] = metrics['top3_scoreline_share'] - float(v0['top3_scoreline_share'])
metrics['awmae_delta_vs_v1'] = metrics['awmae'] - float(v1['awmae'])

metrics['safety_pass'] = (
    (metrics['awmae_gain_vs_v0'] > 0)
    & (metrics['exact_delta_vs_v0'] >= -0.010)
    & (metrics['gd_delta_vs_v0'] >= -0.015)
    & (metrics['top3_delta_vs_v0'] <= 0.030)
    & (metrics['mean_pred_total'].between(1.0, 4.5))
)

gate_candidates = metrics[
    metrics['variant_name'].isin(['V2_hard_helpful_gate', 'V3_conservative_soft_gate'])
    & metrics['safety_pass']
].sort_values('awmae')

if len(gate_candidates):
    selected_row = gate_candidates.iloc[0].to_dict()
    selected_gate_name = str(selected_row['variant_name'])
    decision_code = 'A'
    decision_text = 'Tournament gate improves AW-MAE and passes safety checks.'
else:
    best_row = metrics.sort_values('awmae').iloc[0].to_dict()
    v1_safe = bool(metrics[metrics['variant_name'].eq('V1_full_repair')]['safety_pass'].iloc[0])
    if str(best_row['variant_name']) in ['V2_hard_helpful_gate', 'V3_conservative_soft_gate']:
        decision_code = 'B'
        decision_text = 'A gate variant has best AW-MAE but does not pass safety, so fallback is used.'
        if v1_safe:
            selected_gate_name = 'V1_full_repair'
            selected_row = metrics[metrics['variant_name'].eq('V1_full_repair')].iloc[0].to_dict()
        else:
            selected_gate_name = 'V0_baseline_outcome'
            selected_row = metrics[metrics['variant_name'].eq('V0_baseline_outcome')].iloc[0].to_dict()
    elif str(best_row['variant_name']) == 'V1_full_repair' and v1_safe:
        decision_code = 'C'
        decision_text = 'Full repair remains best and passes safety; selected gate is V1_full_repair.'
        selected_gate_name = 'V1_full_repair'
        selected_row = metrics[metrics['variant_name'].eq('V1_full_repair')].iloc[0].to_dict()
    elif str(best_row['variant_name']) == 'V0_baseline_outcome':
        decision_code = 'D'
        decision_text = 'Baseline outcome remains best.'
        selected_gate_name = 'V0_baseline_outcome'
        selected_row = metrics[metrics['variant_name'].eq('V0_baseline_outcome')].iloc[0].to_dict()
    else:
        decision_code = 'D'
        decision_text = 'No gate or full repair passes safety. Final selected gate is V0_baseline_outcome.'
        selected_gate_name = 'V0_baseline_outcome'
        selected_row = metrics[metrics['variant_name'].eq('V0_baseline_outcome')].iloc[0].to_dict()

selected_gate_cfg = GATE_CONFIGS[selected_gate_name]
selected_decoder = GATE_RESULTS[selected_gate_name]['best_decoder']
selected_gate_config = {
    'experiment': 'EXP09D',
    'selected_gate': selected_gate_name,
    'selected_gate_config': selected_gate_cfg,
    'selected_decoder': selected_decoder,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'v0_awmae': float(v0['awmae']),
    'v1_full_repair_awmae': float(v1['awmae']),
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_v0': float(selected_row.get('awmae_gain_vs_v0', 0.0)),
}

with open(SUM_DIR / 'selected_gate_config.json', 'w', encoding='utf-8') as f:
    json.dump(selected_gate_config, f, indent=2, ensure_ascii=False)

selected_gate_config_df = pd.DataFrame([{'key': k, 'value': str(v)} for k, v in selected_gate_config.items()])
display(metrics.sort_values('awmae').reset_index(drop=True))
display(selected_gate_config_df)
log_saved(SUM_DIR / 'selected_gate_config.json')

final_decision = {
    'experiment': 'EXP09D',
    'base_pipeline': 'EXP05A-LITE-FIX-V2 / EXP09C',
    'main_change': 'tournament-gated outcome repair probability',
    'selected_gate': selected_gate_name,
    'decision_code': decision_code,
    'decision_text': decision_text,
    'v0_awmae': float(v0['awmae']),
    ...
    'selected_awmae': float(selected_row['awmae']),
    'selected_awmae_gain_vs_v0': float(selected_row.get('awmae_gain_vs_v0', 0.0)),
    'caveat': 'Goal and tail predictions stay baseline. Only outcome probability is gated by tournament_structure.',
}
...
# %% [markdown]
# # 16. Final Model Training dan Test Inference
#
# Bagian ini melatih model final pada full train. Test inference memakai selected tournament gate:
# goal/tail tetap baseline EXP05A, outcome baseline dari EXP05A, dan outcome repair dari EXP05A+EXP09A.
...
test_pred_match_best.to_csv(PRED_DIR / 'test_pred_exp09d_best_safe.csv', index=False)
log_saved(PRED_DIR / 'test_pred_exp09d_best_safe.csv')
...
# %% [markdown]
# # 17. Submission Mapping dan Pair Consistency
#
...
submission_path = SUB_DIR / 'submission_exp09d_best_safe.csv'
...
# %% [markdown]
# # 18. Runtime Summary dan Catatan Akhir
#
# Bagian ini menyimpan runtime summary dan menampilkan ringkasan akhir eksperimen. Output utama yang
# perlu dicek adalah selected gate, decision code, validation metric, dan path submission final.
