...
# %% [markdown]
# # 18. EXP12A Submission Export, Sanity Check, dan Final Decision
#
# Bagian ini mengekspor submission utama dari variant yang sedang dijalankan ke struktur
# EXP12A yang rapi. Jika EXP17 aktif dan berhasil, submission utama memakai EXP17.
# Jika tidak, fallback ke EXP15 jika aktif, lalu fallback ke advanced base pipeline.
# Bagian ini juga mengecek id order sample submission, no missing, no duplicate, integer
# non-negative prediction, pair consistency, dan menyimpan final decision sebagai tabel.

# %%
# ============================================================
# 18. EXP12A submission export, sanity check, and final decision
# ============================================================
...
def validate_and_save_exp12a_submission(sub_df: pd.DataFrame, label: str) -> tuple[pd.DataFrame, Path, pd.DataFrame]:
    ...
    # Align to sample order explicitly.
    temp = sample_aligned.copy()
    temp["_id_key"] = temp[sample_id_col if sample_id_col in temp.columns else ID_COL].astype(str)
    sub["_id_key"] = sub[ID_COL].astype(str)
    aligned = temp.merge(
    ...
    id_col_out = sample_id_col if sample_id_col in aligned.columns else ID_COL
    aligned = aligned.rename(columns={id_col_out: ID_COL})
    ...
    checks = [
        ...
        {"variant": label, "check": "pair_consistency", "passed": check_submission_pair_consistency(aligned, test_raw)},
